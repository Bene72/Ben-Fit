...
export default function ExRow({ ex, wId, edit, onUpdate, onDelete, onMove, isFirst, isLast, recentLog }) {
  ...
  // Extraire les secondes de repos pour le chrono
  const restSeconds = (() => {
    const r = ex.rest || ''
  ...
  if (edit) {
    return (
      <div style={{ background: '#FAFBFF', border: '1.5px solid #C5D0F0', borderRadius: '12px', padding: '14px 14px 12px', marginBottom: '10px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '8px', marginBottom: '10px' }}>
          {[
            ...
            <div key={f.field} style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
              <label style={{ fontSize: '10px', letterSpacing: '1px', textTransform: 'uppercase', color: '#6B7A99', fontWeight: '600' }}>{f.label}</label>
        ...
        {/* Note athlète en lecture seule (pour contexte coach) */}
        {recentLog?.note && (
          <div style={{ marginTop: 10, padding: '8px 12px', background: '#F0F7FF', borderRadius: 8, borderLeft: '3px solid #4A6FD4', fontSize: 12 }}>
            <div style={{ fontSize: 10, color: '#6B7A99', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.8px', marginBottom: 3 }}>📝 Dernière note athlète</div>
            <div style={{ color: '#0D1B4E' }}>{recentLog.note}</div>
            {recentLog.date && <div style={{ fontSize: 10, color: '#9BA8C0', marginTop: 3 }}>{new Date(recentLog.date).toLocaleDateString('fr-FR', { day: 'numeric', month: 'short', year: 'numeric' })}</div>}
          </div>
        )}
      </div>
    )
  ...
  return (
    <>
      ...
      <div style={{ padding: '12px 14px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 60px 70px 80px 90px', gap: '6px', alignItems: 'center' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            <div style={{ minWidth: 0 }}>
              ...
              {ex.note && <div style={{ fontSize: '11px', color: '#6B7A99', marginTop: 1 }}>📋 {ex.note}</div>}
            </div>
          </div>
        ...
        {/* Dernière note athlète côté coach */}
        {recentLog && (
          <div style={{ marginTop: 8, padding: '8px 12px', background: '#F5F8FF', borderRadius: 8, borderLeft: '3px solid #4A6FD4', display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: 6 }}>
