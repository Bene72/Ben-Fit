...
export default function ExRow({ ex, wId, edit, onUpdate, onUpdateNote, onDelete, onMove, isFirst, isLast, recentLog }) {
  ...
  if (edit) {
    return (
      <div style={{ background: '#FAFBFF', border: '1.5px solid #C5D0F0', borderRadius: '12px', padding: '14px 14px 12px', marginBottom: '10px' }}>
        ...
        {/* Note coach → sauvegardée dans exercises.note, visible par l'athlète */}
        <div style={{ display: 'flex', flexDirection: 'column', gap: '3px' }}>
          <label style={labelStyle}>
            📋 Note coach <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: '#4A6FD4' }}>(vue par l'athlète)</span>
          </label>
          <textarea
            value={localNote}
            onChange={e => setLocalNote(e.target.value)}
            onBlur={() => onUpdate && onUpdate(wId, ex.id, 'note', localNote)}
  ...
  // ── Mode lecture ──────────────────────────────────────────
  // Note coach = exercises.note
  const displayNote = ex.note || ''
