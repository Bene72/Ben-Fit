...
export default function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const fetchAthleteLogs = async () => {
    ...
    if (data === null) {
      try {
        const r2 = await supabase
          ...
          .limit(500)
        if (!r2.error) data = (r2.data || []).map(r => ({ ...r, notes: r.comment }))
      } catch (e) { console.warn('workout_sessions fetch error:', e) }
    }
    ...
    console.log('[Coach] athleteLogs chargés pour client', clientId, byName)
