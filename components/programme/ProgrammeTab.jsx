...
export default function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const [athleteLogs, setAthleteLogs] = useState({}) // { exercise_name: [log, ...] }

  useEffect(() => {
    if (!clientId) return
    const fetchAthleteLogs = async () => {
      // Essayer workout_logs puis workout_sessions
      let data = null
      ...
      if (!data) return
      // Grouper par exercise_name, garder le dernier log avec une note
      const byName = {}
      data.forEach(log => {
        ...
        // Prendre la note la plus récente non vide
        ...
        // Toujours mettre à jour les perf si pas encore remplies
                    ...
                    if (ex.group_id && ex.group_type !== 'Normal') {
                      ...
                      return (
                        <div key={ex.group_id} style={{ border: `2px solid ${groupColors[ex.group_type]||'#C5D0F0'}`, borderRadius: '10px', marginBottom: '10px', overflow: 'hidden' }}>
                          ...
                          {group.map((e, ei) => <ExRow key={e.id} ex={e} wId={workout.id} edit={isEdit} onUpdate={updateExercise} onDelete={deleteExercise} onMove={moveExercise} isFirst={ei===0} isLast={ei===group.length-1} recentLog={athleteLogs[e.name]} />)}
                        </div>
                      )
                    }
                    rendered.add(ex.id)
                    return <ExRow key={ex.id} ex={ex} wId={workout.id} edit={isEdit} onUpdate={updateExercise} onDelete={deleteExercise} onMove={moveExercise} isFirst={exs.indexOf(ex)===0} isLast={exs.indexOf(ex)===exs.length-1} recentLog={athleteLogs[ex.name]} />
                  })
                })()}
