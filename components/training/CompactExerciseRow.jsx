...
export default function CompactExerciseRow({ exercise, selected, latestLog, onSelect, isMobile }) {
  return (
    <button type="button" onClick={onSelect} style={{
      ...
      <div style={{ width: isMobile ? 50 : 60, height: isMobile ? 50 : 60, borderRadius: 8, overflow: 'hidden', background: '#F0F5FF', border: '1px solid #E0E8F5', flexShrink: 0, position: 'relative' }}>
        {exercise.image_url
          ? <Image src={exercise.image_url} alt={exercise.name} fill sizes={isMobile ? '50px' : '60px'} style={{ objectFit: 'cover' }} />
