      ...
      try {
        ...
        if (np) {
          setPlanForm({
            ...
            high_calories: np.high_calories || '',
            high_protein: np.high_protein || '',
            high_carbs: np.high_carbs || '',
            high_fat: np.high_fat || '',
            low_calories: np.low_calories || '',
            low_protein: np.low_protein || '',
            low_carbs: np.low_carbs || '',
            low_fat: np.low_fat || '',
          })
        }
        setLogs(lg || [])
  ...
  const savePlan = async () => {
    ...
    try {
      const planData = {
        ...
        high_calories: +planForm.high_calories || 0,
        high_protein: +planForm.high_protein || 0,
        high_carbs: +planForm.high_carbs || 0,
        high_fat: +planForm.high_fat || 0,
        low_calories: +planForm.low_calories || 0,
        low_protein: +planForm.low_protein || 0,
              ...
              >
                <span
                  style={{
                    position: 'absolute',
                    top: 3,
                    left: planForm.cyclic_diet ? 23 : 3,
                    width: 18,
                    height: 18,
                    borderRadius: '50%',
                    background: 'white',
              ...
              <div style={{ marginTop: 14 }}>
                {[
                  ...
                  },
                  {
                    ...
                    color: 'var(--danger)',
                    keys: [
                      ['low_calories', '🔥 Calories', '1800'],
                      ['low_protein', '🥩 Protéines', '160'],
                      ['low_carbs', '🌾 Glucides', '120'],
                      ['low_fat', '🥑 Lipides', '65'],
                    ],
                  },
                      ...
                      {keys.map(([key, l, ph]) => (
                        <div key={key}>
                          ...
                          <input
                            ...
                            onChange={(e) => setPlanForm((p) => ({ ...p, [key]: e.target.value }))}
                            placeholder={ph}
                            style={{ ...inp, borderColor: `${color}66` }}
                          />
                        </div>
                      ))}
                    </div>
        ...
        <div>
          <div
            style={{
              ...
              gridTemplateColumns: 'repeat(2,1fr)',
              gap: 12,
              marginBottom: plan.cyclic_diet ? 14 : 0,
            }}
          >
            {[
                    ...
                    <div style={{ display: 'flex', gap: 16, flexWrap: 'wrap' }}>
                      {vals.map(([icon, val, unit]) => (
                        <div key={unit} style={{ textAlign: 'center' }}>
                          ...
                          <div
                            style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: 20, color }}
                          >
                            {val || '—'}
                          </div>
