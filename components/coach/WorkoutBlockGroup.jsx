 ...
 *
 * Ici :
 *  - Mode édition : les mêmes champs structurés que la modale
 *    "Créer un Workout Block" (Format / Rounds / Cap / Repos /
 *    Objectif / Note coach), plus une liste compacte des mouvements
 *    (un input par ligne, comme à la création).
 *  - Mode lecture : rendu identique à ce que voit l'élève
 *    (WorkoutBlockV2 dans components/training/ExerciseBlock.jsx),
 *    pour que le coach voie exactement le même rendu pendant qu'il
 *    programme.
 */
...
export default function WorkoutBlockGroup({ group, wId, edit, onUpdate, onDelete, onMove, onAddExercise }) {
  ...
  const meta = parseMeta(first?.note)
  const tc = WORKOUT_BLOCK_COLORS[meta.type] || '#0D1B4E'

  const updateMeta = (field, value) => {
    if (!first) return
    const next = { ...meta, [field]: value, uiVersion: 2 }
      ...
      <div style={{ borderRadius: 10, overflow: 'hidden', border: `2px solid ${tc}`, boxShadow: '0 4px 12px rgba(13,27,78,0.1)', margin: '8px 10px' }}>
        <div style={{ background: tc, padding: '8px 12px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ color: 'white', fontWeight: 800, fontSize: 11, letterSpacing: '0.5px', textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {meta.type}{meta.cap ? ` — CAP ${meta.cap} min` : ''}
          </div>
          {meta.rounds && meta.rounds > 1 && (
            <div style={{ fontSize: 10, color: 'rgba(255,255,255,0.7)', fontWeight: 600 }}>{meta.rounds} rounds</div>
          )}
        </div>
