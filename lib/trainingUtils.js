...
/**
 * Retrouve les métadonnées JSON d'un Workout Block (type/rounds/cap/rest/…)
 * en scannant TOUTES les lignes du groupe, pas seulement la première.
 *
 * Avant ce correctif, on lisait uniquement `block.exercises[0]?.note` : si le
 * coach réordonnait les mouvements d'un block (flèches ▲▼) et que la ligne
 * porteuse des métadonnées n'était plus en première position, le JSON
 * devenait invisible → `meta.uiVersion` disparaissait → l'interface retombait
 * silencieusement sur l'ancien rendu v1 (liste statique, sans score/journal).
 * En scannant tout le groupe, on retrouve la meta quelle que soit sa position.
 */
export function findGroupMeta(groupExercises) {
  for (const ex of groupExercises || []) {
    if (!ex?.note) continue
    try {
      const m = JSON.parse(ex.note)
      if (m && typeof m === 'object') return m
    } catch {
      // ligne suivante — cette note n'est pas (ou plus) un JSON valide
    }
  }
  return {}
}
...
// Palette harmonisée bleu marine → bleu pastel (au lieu du rouge/orange/vert/
// or d'origine), pour rester cohérent avec le reste de l'interface athlète.
...
// ─── Score de Workout Block selon son Format ──────────────────────────────────
// Le champ demandé à l'athlète change selon le Format choisi par le coach à la
// création (wbForm.type dans ProgrammeTab.jsx) — un "For Time" se score en
// temps, un "AMRAP" en rounds+reps, un "EMOM"/"Tabata"/"Death By" en rounds tenus.
export function scoreFieldsForFormat(format) {
  switch (format) {
    case 'For Time':
    case 'RFT':
      return ['time']
