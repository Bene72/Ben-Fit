...
// Voir lib/rateLimit.js pour le détail.

/**
 * Helper : retourne une Promise<boolean> — true si la requête est BLOQUÉE.
 * Usage dans un handler (⚠️ ne pas oublier le `await`) :
 *   if (await checkRateLimit(req, res, { maxRequests: 5, windowMs: 60000 })) return
 */
