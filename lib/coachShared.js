...
/** Boutons avec variant nommé — primary | gold | outline | ghost | danger */
export const btnVariant = (variant) => {
  const map = {
    primary: {
      ...
      color: 'white',
      border: 'none',
      boxShadow: '0 2px 8px rgba(13,27,78,0.18)',
    },
    // Accent premium pour l'action la plus importante d'un écran (ex: "Créer",
    // "Publier", "Activer le cycle") — à utiliser avec parcimonie, 1 par écran.
    gold: {
      background: 'var(--gold, #e8c84a)',
      color: 'var(--navy, #0D1B4E)',
      border: 'none',
      boxShadow: '0 2px 8px rgba(232,200,74,0.35)',
    },
    outline: {
      background: 'transparent',
      color: 'var(--navy, #0D1B4E)',
      border: '1.5px solid var(--navy, #0D1B4E)',
    },
    ghost: { background: 'transparent', color: 'var(--text-soft, #6B7A99)', border: 'none' },
    danger: { background: 'var(--danger, #C45C3A)', color: 'white', border: 'none' },
  }
  return {
    padding: '8px 16px',
    borderRadius: 'var(--r-md, 8px)',
 ...
 *
 * CORRECTIF AUDIT 09/09/2026 (point S2) : cette fonction était dupliquée
 * quasi à l'identique dans components/coach/GestionTab.jsx, avec une
 * différence importante — seule la copie de GestionTab vérifiait que
 * `session` existait avant de lire `session.access_token`. Cette version
 * unique (importée désormais par GestionTab.jsx) reprend cette vérification.
 *
