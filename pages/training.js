...
export default function Training() {
  ...
  const addSuperset = async (workoutId) => {
    ...
    const exA = await supabase.from('exercises').insert({ workout_id: workoutId, name: 'Exercice A', sets: 3, reps: '10', rest: '60s', note: '', target_weight: '', order_index: order, group_type: 'Superset', group_id: gid }).select().single()
    const exB = await supabase.from('exercises').insert({ workout_id: workoutId, name: 'Exercice B', sets: 3, reps: '10', rest: '90s', note: '', target_weight: '', order_index: order + 1, group_type: 'Superset', group_id: gid }).select().single()
    if (exA.data && exB.data) setWorkouts(prev => prev.map(w => w.id === workoutId ? { ...w, exercises: [...(w.exercises || []), exA.data, exB.data] } : w))
  }
  ...
  const addGiantSet = async (workoutId) => {
    ...
    const order = w?.exercises?.length || 0
    const gid = Date.now().toString()
    const inserts = ['Exercice A', 'Exercice B', 'Exercice C'].map((name, i) =>
      supabase.from('exercises').insert({ workout_id: workoutId, name, sets: 3, reps: '10', rest: i === 2 ? '90s' : '0s', note: '', target_weight: '', order_index: order + i, group_type: 'Giant Set', group_id: gid }).select().single()
    )
    const results = await Promise.all(inserts)
    const newExs = results.map(r => r.data).filter(Boolean)
    if (newExs.length) setWorkouts(prev => prev.map(w => w.id === workoutId ? { ...w, exercises: [...(w.exercises || []), ...newExs] } : w))
  }
