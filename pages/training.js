...
export default function TrainingPage() {
  ...
  const groupedBlocks = useMemo(() => {
    ...
    return Array.from(groups.entries()).map(([label, exercises]) => ({ label, exercises }))
  }, [selectedWorkout])
          ...
          <div className="ui-stack">
            <div className="ui-card ui-card--soft ui-card--padded">
              ...
              <div className="ui-muted" style={{ lineHeight: 1.6 }}>
                {[exercise.sets && `${exercise.sets} séries`, exercise.reps && `${exercise.reps} reps`, exercise.rest && `${exercise.rest} repos`]
                  .filter(Boolean)
                ...
                >
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setLogForm((prev) => ({ ...prev, weight_used: e.target.value }))}
                  ...
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) => setLogForm((prev) => ({ ...prev, reps_done: e.target.value }))}
                      placeholder="ex. 10"
                    />
                  </div>
                  <div>
                    ...
                    <input
                      className="ui-input"
                      value={logForm.rpe}
                      onChange={(e) => setLogForm((prev) => ({ ...prev, rpe: e.target.value }))}
                      placeholder="ex. 8"
                    />
                  </div>
                </div>
            ...
            {exerciseLatestLog ? (
              <div className="ui-card ui-card--padded">
                ...
                <div className="ui-muted" style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                  {exerciseLatestLog.weight_used ? <span style={{ fontWeight: 700, color: '#0D1B4E' }}>{exerciseLatestLog.weight_used}</span> : null}
                  ...
                  {exerciseLogsList.slice(0, 5).map((log) => (
                    <div key={log.id} className="ui-list-item" style={{ padding: '10px 12px' }}>
                      <div>
                        ...
                        <div className="ui-muted" style={{ fontSize: 12 }}>{formatLogDate(log)}</div>
                        {log.comment ? <div className="ui-muted" style={{ fontSize: 12, marginTop: 4 }}>{log.comment}</div> : null}
                      </div>
                    </div>
                  ))}
                </div>
  ...
  if (!workouts.length) {
    return (
      <AppShell title="Training" subtitle="Aucune séance disponible pour le moment.">
        <EmptyPanel title="Aucune séance trouvée" description="Ton coach n'a pas encore ajouté de séance à ton programme." />
      </AppShell>
    )
  }
      ...
      >
        <div className="ui-stack">
          <SurfaceCard padded sticky={!isMobile}>
            <SectionHead title="Séances" caption="Choisis la séance et navigue bloc par bloc." />
                ...
                return (
                  ...
                  >
                    <div>
                      ...
                      <div className="ui-muted" style={{ fontSize: 13 }}>{(w.exercises || []).length} exercice(s)</div>
                    </div>
        ...
        <div className="ui-stack">
          <SurfaceCard padded>
            ...
            {groupedBlocks.length ? (
              ...
              <EmptyPanel title="Séance vide" description="Ton coach n'a pas encore structuré cette séance." />
            )}
          </SurfaceCard>
        </div>
      </div>
