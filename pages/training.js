...
export default function TrainingPage() {
  ...
  const selectedLogs = useMemo(
    () => (selectedExercise ? logsByExerciseName[selectedExercise.name] || [] : []),
    [selectedExercise, logsByExerciseName]
  )
  ...
  const shellLayout =
    activeTab === 'session'
      ? {
          display: 'grid',
          gridTemplateColumns: isMobile ? '1fr' : 'minmax(250px, 0.92fr) minmax(420px, 1.4fr) minmax(260px, 0.92fr)',
          gap: 18,
          alignItems: 'start',
        }
      : {
        ...
        <div style={shellLayout}>
                  ...
                  return (
                    <button
                      ...
                      style={{
                        ...
                        textAlign: 'left',
                        borderRadius: 16,
                        border: active ? '1.5px solid #2C64E5' : '1px solid #DCE5F3',
                      ...
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10 }}>
                        <div>
                          ...
                          <div style={{ color: '#6B7A99', fontSize: 14 }}>
                            {getWorkoutDayLabel(workout.day_of_week)} · {(workout.exercises || []).length} exercice(s)
                          </div>
                        </div>
          ...
          <SurfaceCard padded>
            <SectionHead
              ...
              caption="Sélectionne un exercice depuis la liste. Le détail s’affiche dans l’espace de travail juste en dessous."
                ...
                {exerciseBlocks.map((block) => {
                  if (block.kind === 'group') {
                    return (
                      <div
                        ...
                        style={{
                          borderRadius: 18,
                          border: '2px solid rgba(143,160,122,0.82)',
                          overflow: 'hidden',
                          background: 'white',
                        }}
                      >
                  ...
                  return (
                    <ExerciseRow
                      ...
                      selected={selectedExerciseId === block.exercise.id}
                      latestLog={(logsByExerciseName[block.exercise.name] || [])[0]}
                      onSelect={() => setSelectedExerciseId(block.exercise.id)}
                    />
                  )
                })}
            ...
            {selectedExercise ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {selectedExercise.image_url ? (
                  <button
                    type="button"
                    onClick={() => setImageLightbox(selectedExercise.image_url)}
                    style={{
                      padding: 0,
                ...
                <InfoCard
                  ...
                  value={selectedLatestLog ? `${latestPerfText(selectedLatestLog)} · ${safeDateLabel(selectedLatestLog.logged_at)}` : 'Aucune donnée pour cet exercice.'}
                />
                      ...
                      {selectedLogs.slice(0, 8).map((log) => (
                        <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 8, alignItems: 'center' }}>
                          <div>
                            ...
                            <div style={{ color: '#6B7A99', fontSize: 12 }}>{safeDateLabel(log.logged_at)}</div>
                          </div>
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '88px minmax(0,1fr) auto', gap: 14, alignItems: 'center' }}>
        ...
        <div>
          ...
          {latestLog ? (
            <div style={{ color: '#6B7A99', marginTop: 10, fontSize: 13 }}>Dernier log : {latestPerfText(latestLog)}</div>
          ) : null}
        </div>

        <div style={{ color: selected ? '#2C64E5' : '#9AA8C2', fontWeight: 800, fontSize: 18 }}>
          {selected ? '●' : '○'}
        </div>
      </div>
    </button>
...
function ExerciseWorkspace({ exercise, input, onInput, onLog, logging, onImageOpen, latestLog, isMobile }) {
  return (
    ...
    >
      <SectionHead
        title={exercise.name}
        caption="Zone d’exécution : détail exercice, note coach, saisie charge / reps et commentaire."
        action={<StatusBadge tone="accent">Ouvert</StatusBadge>}
      />

      <div
        style={{
          display: 'grid',
          gridTemplateColumns: isMobile ? '1fr' : 'minmax(260px, 0.95fr) minmax(0, 1.05fr)',
          gap: 16,
          alignItems: 'start',
        }}
      >
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <div style={{ border: '1px solid #DCE5F3', borderRadius: 18, background: '#FFFFFF', padding: 14 }}>
            ...
            <div style={{ color: '#6B7A99', lineHeight: 1.7, fontSize: 14 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          <div style={{ border: '1px solid #DCE5F3', borderRadius: 18, background: '#FFFFFF', padding: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(3, minmax(0, 1fr))', gap: 12 }}>
              <Field label="Charge / poids">
                <input value={input.weight || ''} onChange={(e) => onInput('weight', e.target.value)} placeholder="ex. 60 kg" style={inputStyle()} />
              </Field>
              <Field label="Reps faites">
                <input value={input.reps || ''} onChange={(e) => onInput('reps', e.target.value)} placeholder="ex. 10" style={inputStyle()} />
              </Field>
              <Field label="RPE">
                <input value={input.rpe || ''} onChange={(e) => onInput('rpe', e.target.value)} placeholder="ex. 8" style={inputStyle()} />
              </Field>
            </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, alignItems: 'center', marginTop: 12, flexDirection: isMobile ? 'column' : 'row' }}>
              <div style={{ color: '#6B7A99', fontSize: 13, lineHeight: 1.6 }}>
                Tu peux logger librement charge, reps et ressenti pour garder une vraie trace exploitable.
              </div>
              <button
                type="button"
                onClick={onLog}
          ...
          <InfoCard
            ...
            value={latestLog ? `${latestPerfText(latestLog)} · ${safeDateLabel(latestLog.logged_at)}${latestLog.note ? ` · ${latestLog.note}` : ''}` : 'Aucun log encore enregistré pour cet exercice.'}
          />
        </div>
      </div>
    </div>
...
function MiniKpi({ label, value }) {
  return (
    ...
    >
      <div style={{ fontSize: 11, color: '#6B7A99', textTransform: 'uppercase', letterSpacing: '1.1px', marginBottom: 6 }}>{label}</div>
      <div style={{ fontWeight: 900, fontSize: 28, color: '#0D1B4E' }}>{value}</div>
    </div>
  )
}
...
function InfoCard({ title, value }) {
  return (
    ...
    >
      <div style={{ fontWeight: 900, color: '#0D1B4E', marginBottom: 8 }}>{title}</div>
      <div style={{ color: '#6B7A99', lineHeight: 1.7, fontSize: 14 }}>{value}</div>
    </div>
  )
}
