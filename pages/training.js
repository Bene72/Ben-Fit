...
export default function TrainingPage() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        // Prefill depuis les valeurs coach (target_weight, reps) — pas depuis le dernier log
        // Chaque jour est indépendant : on part des valeurs prescrites
        const prefills = {}
  ...
  async function logPerformance(exercise) {
    ...
    try {
      ...
      // Reset inputs à vide après log — chaque série est indépendante
              ...
              <SurfaceCard padded sticky>
                ...
                {selectedExercise ? (
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ border: '1px solid #DCE5F3', borderRadius: 12, background: '#FFFFFF', padding: 12 }}>
                      ...
                      {(logsByExerciseName[selectedExercise?.name] || []).length === 0 ? (
                        <div style={{ color: '#9AAAD4', fontSize: 13 }}>Aucun log pour cet exercice.</div>
                      ) : (
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {(logsByExerciseName[selectedExercise?.name] || []).slice(0, 5).map((log, i) => (
                            <div key={log.id || i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0', borderBottom: i < 4 && i < (logsByExerciseName[selectedExercise?.name]||[]).length - 1 ? '1px solid #F0F5FF' : 'none' }}>
                              <div style={{ width: 6, height: 6, borderRadius: '50%', background: i === 0 ? '#2C64E5' : '#C5D8F5', flexShrink: 0 }} />
                              <div style={{ flex: 1 }}>
                                <span style={{ fontWeight: i === 0 ? 800 : 600, fontSize: i === 0 ? 14 : 12, color: i === 0 ? '#2C64E5' : '#4A6FB5' }}>
                                  {latestPerfText(log)}
                                </span>
                                {getLogNote(log) && <span style={{ fontSize: 10, color: '#9AAAD4', marginLeft: 5 }}>· {getLogNote(log)}</span>}
                              </div>
...
function ExerciseWorkspace({ exercise, input, onInput, onLog, logging, onImageOpen, latestLog, isMobile, recentLogs }) {
  return (
    <div style={{ borderRadius: 10, border: '1.5px solid #2C64E5', background: '#F8FBFF', padding: isMobile ? 12 : 16, marginBottom: 8 }}>
      {exercise.note && (
        <div style={{ marginBottom: 10, padding: '7px 11px', background: 'white', borderRadius: 8, borderLeft: '3px solid #2C64E5', fontSize: 13, color: '#0D1B4E', lineHeight: 1.6 }}>
          📋 <span style={{ fontWeight: 600 }}>Note coach :</span> {exercise.note}
        </div>
      )}
