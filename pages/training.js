      ...
      {imageLightbox && (
        ...
        >
          <img
            src={imageLightbox}
            alt="Exercice"
            style={{ maxWidth: 'min(1000px, 100%)', maxHeight: '88vh', borderRadius: 16 }}
          />
        </div>
      )}
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      ...
      <div
        style={{
          ...
          overflow: 'hidden',
          boxShadow: '0 2px 8px rgba(13,27,78,0.06)',
        }}
      >
        <div
          style={{
            ...
            {weekOffset === 0
              ...
              : weekOffset === -1
                ...
                : weekOffset === 1
                  ? 'Semaine prochaine'
                  : weekLabel(weekDays)}
            <div style={{ fontSize: 10, opacity: 0.65, fontWeight: 400, marginTop: 1 }}>
              {weekLabel(weekDays)}
            </div>
          </div>
          <CalNavBtn
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: 'none',
                  cursor: 'pointer',
                  background: isSelected ? 'var(--accent)' : isToday ? '#EEF4FF' : 'transparent',
              ...
              >
                {hasNote && (
                  <span
                    style={{ position: 'absolute', top: 2, right: 4, fontSize: 9, lineHeight: 1 }}
                    title="Annotation"
                  >
                    📌
                  </span>
                ...
                <span
                  style={{
                    ...
                    color: isSelected ? 'rgba(255,255,255,0.75)' : '#6B7A99',
                  }}
                >
                ...
                <div style={{ display: 'flex', gap: 2, height: 6, alignItems: 'center' }}>
                  {hasWorkout && (
                    <Dot color={isSelected ? 'rgba(255,255,255,0.7)' : 'var(--accent)'} />
                  )}
      ...
      {selectedCalDay && (
        ...
        >
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            <button
              onClick={() => saveNote(selectedCalDay)}
              disabled={savingNote}
              style={{
                border: 'none',
            ...
            {calendarNotes[selectedCalDay] && (
              <button
                onClick={() => removeNote(selectedCalDay)}
                disabled={savingNote}
                style={{
            ...
            <button
              onClick={() => selectCalDay(selectedCalDay)}
              style={{
                border: 'none',
                background: 'transparent',
                color: '#6B7A99',
            ...
            {calDayWorkouts.map((workout) => (
              <button
                ...
                style={{
                  width: '100%',
                  textAlign: 'left',
                  background: openWorkout === workout.id ? 'var(--accent)' : 'white',
                  borderRadius: 10,
                  border: openWorkout === workout.id ? 'none' : '1px solid #DCE5F3',
                  padding: '10px 12px',
                  cursor: 'pointer',
                  fontFamily: "'DM Sans',sans-serif",
                }}
              >
                <div
                  style={{
                    fontWeight: 800,
                    fontSize: 14,
                    color: openWorkout === workout.id ? 'white' : 'var(--navy)',
                    marginBottom: 2,
                  }}
                >
                  {workout.name}
                ...
                <div
                  style={{
                    fontSize: 11,
                    color: openWorkout === workout.id ? 'rgba(255,255,255,0.7)' : '#6B7A99',
                  }}
                >
                  {(workout.exercises || []).length} exercices · {workout.duration || '—'} min
                </div>
              </button>
            ))}
          </div>
              ...
              return (
                <button
                  ...
                  style={{
                    ...
                    textAlign: 'left',
                    borderRadius: 10,
                    border: active ? '1.5px solid var(--accent)' : '1px solid #DCE5F3',
        ...
        {selectedExercise ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {selectedExercise.image_url && (
              <button
                type="button"
                onClick={() => onImageOpen(selectedExercise.image_url)}
                style={{
                  padding: 0,
                  border: '1px solid #DCE5F3',
                  borderRadius: 12,
                  ...
                  {(logsByExerciseName[selectedExercise.name] || []).slice(0, 5).map((log, i) => (
                    <div
                      ...
                      style={{
                        ...
                        gap: 6,
                        padding: '4px 0',
                        borderBottom: i < 4 ? '1px solid #F0F5FF' : 'none',
                      }}
                    >
                      ...
                      <div style={{ fontSize: 10, color: '#9AAAD4', flexShrink: 0 }}>
                        {safeDateLabel(getLogDate(log))}
                      </div>
                    </div>
                  ))}
                </div>
    ...
    >
      <div style={{ padding: '12px 14px 8px', borderBottom: '1px solid #E8ECF5' }}>
        ...
        <div style={{ fontSize: 11, color: '#6B7A99', marginTop: 2 }}>
          Toutes tes séances du cycle actuel
        </div>
      </div>
          ...
          return (
            <button
              ...
              style={{
                ...
                textAlign: 'left',
                borderRadius: 10,
                border: isActive ? '1.5px solid var(--accent)' : '1px solid #DCE5F3',
                background: isActive ? '#F5F8FF' : '#FAFBFF',
                padding: '10px 12px',
                cursor: 'pointer',
                fontFamily: "'DM Sans',sans-serif",
                display: 'flex',
            ...
            >
              <div
                style={{
                  ...
                  height: 38,
                  borderRadius: 10,
                  background: isActive ? 'var(--accent)' : '#EEF4FF',
                  display: 'flex',
                  flexDirection: 'column',
                  alignItems: 'center',
                  justifyContent: 'center',
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                <div style={{ color: '#6B7A99', fontSize: 11, marginTop: 1 }}>
                  {getWorkoutDayLabel(workout.day_of_week)} · {(workout.exercises || []).length}{' '}
                  exos{workout.duration ? ` · ${workout.duration} min` : ''}
                </div>
              </div>
...
function CycleBadge({ name, style }) {
  return (
    ...
    >
      <span style={{ fontSize: 11, color: 'var(--accent)', fontWeight: 700 }}>🏆 Cycle actuel</span>
      <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--navy)' }}>{name}</div>
    </div>
  )
}
...
function MiniKpi({ label, value }) {
  return (
    <div
      style={{ border: '1px solid #C5D8F5', borderRadius: 10, background: '#EEF4FF', padding: 10 }}
    >
      <div
        style={{
          fontSize: 10,
      ...
      <div style={{ fontWeight: 900, fontSize: 22, color: 'var(--navy)' }}>{value}</div>
    </div>
  )
}
...
function InfoCard({ title, value }) {
  return (
    <div
      style={{ border: '1px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 12 }}
    >
      <div
        style={{
          fontWeight: 800,
