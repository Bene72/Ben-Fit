...
import { fetchTrainingPageData, createWorkout, removeWorkout, createExercise, createExercisesBatch, updateExerciseField, removeExercise, createWorkoutLog, markWorkoutSessionDone, saveWorkoutComment, getExerciseLogKey } from '../lib/services/workouts'
...
export default function Training() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        setUser(user)
        const { workouts, sessions, logsByExercise } = await fetchTrainingPageData(user.id)
        setWorkouts(workouts)
        setSessions(sessions)
        const comments = {}
        ;(sessions || []).forEach(s => { if (s.athlete_comment) comments[s.workout_id] = s.athlete_comment })
  ...
  const loadWorkouts = async (uid) => {
    try {
      setError('')
      const { workouts } = await fetchTrainingPageData(uid)
      setWorkouts(workouts)
    } catch (e) {
  ...
  const addWorkout = async () => {
    ...
    try {
      setError('')
      const data = await createWorkout({ ...newWorkout, client_id: user.id })
      if (data) {
  ...
  const addSuperset = async (workoutId) => {
    ...
    try {
      setError('')
      const newExs = await createExercisesBatch([
        { ...base, name: 'Exercice A', order_index: order },
        { ...base, name: 'Exercice B', order_index: order + 1 },
      ])
      if (newExs.length === 2) {
        setWorkouts(prev => prev.map(w => w.id === workoutId ? { ...w, exercises: [...(w.exercises || []), ...newExs] } : w))
      }
    } catch (e) {
      setError(e.message || 'Impossible d'ajouter le superset')
    }
  }
  ...
  const addGiantSet = async (workoutId) => {
    ...
    try {
      ...
      setError(e.message || 'Impossible d'ajouter le giant set')
    }
  }
  ...
  const logPerformance = async (exercise) => {
    ...
    try {
      setError('')
      const data = await createWorkoutLog(entry)
      if (data) {
        const key = getExerciseLogKey(exercise)
        setWorkoutLogs(prev => ({
          ...prev,
          [key]: [data, ...(prev[key] || [])]
        }))
  ...
  const saveComment = async (workoutId, comment) => {
    ...
    try {
      ...
      const today = new Date().toISOString().split('T')[0]
      const data = await saveWorkoutComment({
        sessionId: session?.id,
        payload: { client_id: user.id, workout_id: workoutId, date: today, completed: session?.completed || false, athlete_comment: comment }
      })
      if (data && !session) setSessions(prev => [...prev, data])
      if (data && session) setSessions(prev => prev.map(s => s.id === session.id ? data : s))
    } catch (e) {
