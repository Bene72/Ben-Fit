...
async function loadLogsForClient(currentUserId) {
  const { data, error } = await supabase
    .from('workout_logs')
    .select('id, client_id, exercise_name, weight_used, reps_done, notes, note, logged_at')
    .eq('client_id', currentUserId)
    .order('logged_at', { ascending: false })
    .limit(500)
  if (error) { console.warn('workout_logs error:', error.message); return [] }
  return (data || []).map(row => ({
    ...row,
...
export default function TrainingPage() {
    ...
    async function boot() {
      try {
        ...
        const [{ data: workoutData, error: workoutError }, logsData] = await Promise.all([
          supabase.from('workouts').select('*').eq('client_id', currentUser.id).eq('is_archived', false).order('day_of_week', { ascending: true }),
          loadLogsForClient(currentUser.id),
        ])

        if (workoutError) throw workoutError

        // Fetch séparé des exercices (contourne les limitations RLS sur les jointures imbriquées)
        ...
        if (archivedData?.length) {
          const archivedIds = archivedData.map(w => w.id)
          const { data: archivedExData } = await supabase
            .from('exercises').select('*').in('workout_id', archivedIds).order('order_index')
          const archivedEx = archivedExData || []
          setArchivedWorkouts(archivedData.map(w => ({
            ...w,
            exercises: archivedEx.filter(ex => ex.workout_id === w.id),
          })))
        } else {
          setArchivedWorkouts([])
        }
        ...
        const mapped = (workoutData || []).map((workout) => ({
          ...workout,
          exercises: normalizeExercises(allExercises.filter(ex => ex.workout_id === workout.id)),
        }))
        const groupedLogs = {}
  ...
  async function logPerformance(exercise) {
    ...
    try {
      ...
      // La note est conservée — elle est liée à l'exercice, pas à la série
      setLogInputs((prev) => ({
        ...prev,
        [exercise.id]: {
          weight: input.weight,
          reps: input.reps,
          rpe: '',
          note: input.note || ''   // ← conservée
        }
      }))
      
      saveInputToLocalStorage(exercise.id, { weight: input.weight, reps: input.reps, rpe: '', note: input.note || '' }, user?.id)
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                          {(logsByExerciseName[selectedExercise?.name] || []).slice(0, 5).map((log, i) => (
                            <div key={log.id || i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', borderBottom: i < 4 ? '1px solid #F0F5FF' : 'none' }}>
                              <div style={{ width: 6, height: 6, borderRadius: '50%', background: i === 0 ? '#2C64E5' : '#C5D8F5', flexShrink: 0 }} />
                              <div style={{ flex: 1 }}>
          ...
          <div style={{ marginTop: 8 }}>
            <Field label="Notes">
              <input
                ...
                placeholder="Sensation, tempo, commentaire…"
                style={inputStyle()}
              />
            </Field>
          </div>
...
// ─── HEATMAP D'ADHÉRENCE ────────────────────────────────────────
function AdherenceHeatmap({ logsByExerciseName }) {
  const now = new Date()
  const [heatmapYear,  setHeatmapYear]  = useState(now.getFullYear())
  const [heatmapMonth, setHeatmapMonth] = useState(now.getMonth() + 1)

  const { daysWithLogs, daysInMonth, firstDay } = useMemo(() => {
    const target = `${heatmapYear}-${String(heatmapMonth).padStart(2, '0')}`
    const days = new Set()
    Object.values(logsByExerciseName).forEach(logs => {
      logs.forEach(log => {
        const d = log.logged_at || log.created_at || log.date
        if (d && d.startsWith(target)) {
          days.add(parseInt(d.split('-')[2]))
        }
      })
    })
    const dim = new Date(heatmapYear, heatmapMonth, 0).getDate()
    const fd  = new Date(heatmapYear, heatmapMonth - 1, 1).getDay()
    return { daysWithLogs: days, daysInMonth: dim, firstDay: fd === 0 ? 6 : fd - 1 }
  }, [logsByExerciseName, heatmapYear, heatmapMonth])

  const monthLabel = new Date(heatmapYear, heatmapMonth - 1, 1)
  ...
  const nextMonth = () => {
    const n = new Date()
    if (heatmapYear > n.getFullYear() || (heatmapYear === n.getFullYear() && heatmapMonth >= n.getMonth() + 1)) return
    if (heatmapMonth === 12) { setHeatmapMonth(1); setHeatmapYear(y => y + 1) }
    else setHeatmapMonth(m => m + 1)
  }
  const isCurrentMonth = heatmapYear === now.getFullYear() && heatmapMonth === now.getMonth() + 1
  const adherencePct = Math.round((daysWithLogs.size / daysInMonth) * 100)

  return (
    <div style={{ background: 'white', borderRadius: 14, border: '1px solid #DCE5F3', padding: '16px', marginBottom: 12 }}>
      {/* Header */}
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 12 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          <span style={{ fontSize: 18 }}>🔥</span>
          <div>
            <div style={{ fontWeight: 800, fontSize: 13, color: '#0D1B4E' }}>Adhérence</div>
            <div style={{ fontSize: 10, color: '#9BA8C0' }}>{daysWithLogs.size} séances · {adherencePct}% du mois</div>
          </div>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
          <button onClick={prevMonth} style={{ background: '#EEF4FF', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontWeight: 700, color: '#2C64E5', fontSize: 14 }}>‹</button>
          <span style={{ fontSize: 12, fontWeight: 700, color: '#0D1B4E', minWidth: 100, textAlign: 'center', textTransform: 'capitalize' }}>{monthLabel}</span>
          <button onClick={nextMonth} disabled={isCurrentMonth} style={{ background: isCurrentMonth ? '#F5F5F5' : '#EEF4FF', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: isCurrentMonth ? 'default' : 'pointer', fontWeight: 700, color: isCurrentMonth ? '#CCC' : '#2C64E5', fontSize: 14 }}>›</button>
        </div>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3, marginBottom: 3 }}>
        {['L','M','M','J','V','S','D'].map((d, i) => (
          <div key={i} style={{ fontSize: 9, textAlign: 'center', color: '#9BA8C0', fontWeight: 700 }}>{d}</div>
        ))}
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: 3 }}>
        {Array(firstDay).fill(null).map((_, i) => (
          <div key={`e${i}`} style={{ aspectRatio: '1', borderRadius: 5 }} />
        ))}
        {Array(daysInMonth).fill(null).map((_, i) => {
          const day = i + 1
          const has = daysWithLogs.has(day)
          const isToday = isCurrentMonth && day === now.getDate()
          return (
            <div key={day} style={{
              aspectRatio: '1', borderRadius: 5,
              background: has ? '#8FA07A' : isToday ? '#EEF4FF' : '#F5F7FF',
              border: isToday ? '1.5px solid #2C64E5' : '1px solid transparent',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
              fontSize: 9, fontWeight: has ? 800 : 400,
              color: has ? 'white' : '#9BA8C0',
            }}>
              {day}
            </div>
          )
      ...
      {/* Légende + barre progression */}
      <div style={{ marginTop: 10, display: 'flex', flexDirection: 'column', gap: 6 }}>
        <div style={{ height: 5, background: '#EEF4FF', borderRadius: 3, overflow: 'hidden' }}>
          <div style={{ height: '100%', background: 'linear-gradient(90deg, #4A6FD4, #8FA07A)', width: `${adherencePct}%`, borderRadius: 3, transition: 'width 0.5s ease' }} />
        </div>
        <div style={{ display: 'flex', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#6B7A99' }}>
            <div style={{ width: 8, height: 8, borderRadius: 2, background: '#8FA07A' }} /> Séance loguée
          </div>
          <div style={{ fontSize: 10, fontWeight: 700, color: adherencePct >= 70 ? '#8FA07A' : adherencePct >= 40 ? '#4A6FD4' : '#C45C3A' }}>
            {adherencePct >= 70 ? '🔥 Excellent' : adherencePct >= 40 ? '💪 Bien' : '📈 Continue'}
          </div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, fontSize: 10, color: '#6B7A99' }}>
            <div style={{ width: 8, height: 8, borderRadius: 2, background: '#F5F7FF', border: '1px solid #C5D0F0' }} /> Repos
          </div>
        </div>
      </div>
    </div>
...
function HistoryCalendar({ weekDays, weekOffset, setWeekOffset, todayStr, logsByExerciseName, workoutByJsDay, archivedWorkouts, isMobile }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <AdherenceHeatmap logsByExerciseName={logsByExerciseName} />
