...
export default function TrainingPage() {
  ...
  useEffect(() => {
    ...
    async function boot() {
      try {
        ...
        const [{ data: workoutData, error: workoutError }, { data: logsData, error: logsError }] = await Promise.all([
          ...
          supabase.from('workout_logs').select('*').eq('client_id', currentUser.id).order('logged_at', { ascending: false }).limit(300),
        ])
  ...
  async function logPerformance(exercise) {
    ...
    try {
      ...
      const { data, error: insertError } = await supabase.from('workout_logs').insert(payload).select().single()
      if (insertError) throw insertError

      setLogsByExerciseName((prev) => ({ ...prev, [exercise.name]: [data, ...(prev[exercise.name] || [])] }))
            ...
            <SurfaceCard padded sticky>
              ...
              {selectedExercise ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  ...
                  <InfoCard title="Dernière entrée" value={selectedLatestLog ? `${latestPerfText(selectedLatestLog)} · ${safeDateLabel(selectedLatestLog.logged_at)}` : 'Aucune donnée pour cet exercice.'} />
    ...
    <div style={{ borderRadius: 20, border: '1px solid #DCE5F3', background: '#F8FBFF', padding: 18, marginBottom: 12 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(260px, 0.95fr) minmax(0, 1.05fr)', gap: 16, alignItems: 'start' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <InfoCard title="Dernière performance" value={latestLog ? `${latestPerfText(latestLog)} · ${safeDateLabel(latestLog.logged_at)}${getLogNote(latestLog) ? ` · ${getLogNote(latestLog)}` : ''}` : 'Aucun log encore enregistré pour cet exercice.'} />
        </div>
      </div>
    </div>
  )
