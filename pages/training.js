    ...
    <div style={{ borderRadius: 10, border: '1.5px solid #2C64E5', background: '#F8FBFF', padding: isMobile ? 12 : 16, marginBottom: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 12 }}>
        <div>
          ...
          {/* Historique des logs précédents */}
          {recentLogs && recentLogs.length > 0 && (
            <div>
              <div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 6, fontSize: 10, letterSpacing: '0.5px' }}>HISTORIQUE</div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6, maxHeight: 160, overflowY: 'auto' }}>
                {recentLogs.slice(0, 5).map((log, i) => (
                  <div key={log.id || i} style={{ background: 'white', borderRadius: 8, padding: '7px 10px', border: '1px solid #DCE5F3', fontSize: 12 }}>
                    ...
                    {(log.notes || log.note) && (
                      <div style={{ fontSize: 11, color: '#6B7A99', marginTop: 3, fontStyle: 'italic' }}>"{log.notes || log.note}"</div>
                    )}
                  </div>
                ))}
              </div>
          ...
          <div style={{ marginBottom: 8 }}>
            <Field label="Ton ressenti (optionnel)">
              <textarea
                ...
                placeholder="Ex: bonne séance, sensation de force, douleur épaule…"
                rows={2}
