...
export default function TrainingPage() {
  ...
  return (
    <AppShell title="Training" subtitle="Un espace clair et lisible" actions={<SegmentTabs items={TRAINING_TABS} value={activeTab} onChange={setActiveTab} />}>
      ...
      {activeTab === 'session' ? (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(200px, 0.92fr) minmax(380px, 1.4fr) minmax(220px, 0.92fr)', gap: isMobile ? 6 : 14, alignItems: 'start' }}>
            ...
            {workouts.length ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                {workouts.map((workout) => {
                  ...
                  return (
                    <button key={workout.id} type="button" onClick={() => openSession(workout.id)} style={{ width: '100%', textAlign: 'left', borderRadius: isMobile ? 8 : 12, border: active ? '1.5px solid #2C64E5' : '1px solid #DCE5F3', background: active ? '#F5F8FF' : '#FFFFFF', padding: isMobile ? '8px 10px' : '14px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                {exerciseBlocks.map((block) => {
                  ...
                  if (block.kind === 'group' && block.groupType === 'Workout Block') {
                    ...
                    return (
                      <div key={block.id} style={{ borderRadius: 10, overflow: 'hidden', border: `2px solid ${tc}`, boxShadow: '0 4px 12px rgba(13,27,78,0.1)', marginBottom: 2 }}>
                        <div style={{ background: tc, padding: '6px 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
                          <span style={{ fontSize: 12 }}>🔥</span>
                          <div style={{ color: 'white', fontWeight: 800, fontSize: 10, letterSpacing: '0.5px', textTransform: 'uppercase' }}>{meta.type || 'Workout Block'}{meta.cap ? ` — CAP ${meta.cap} min` : ''}{meta.rounds && meta.rounds > 1 ? ` · ${meta.rounds} rounds` : ''}</div>
                        </div>
                        <div style={{ background: '#0D1B4E', padding: '6px 10px' }}>
                          {block.exercises.map((e, i) => (<div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 4, padding: '2px 0', borderBottom: i < block.exercises.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}><span style={{ color: tc, fontSize: 9, fontWeight: 800, minWidth: 10 }}>•</span><span style={{ color: 'white', fontSize: 11, fontWeight: 500 }}>{e.name}</span></div>))}
                        </div>
                      </div>
                    )
                  }
                  ...
                  if (block.kind === 'group') {
                    ...
                    return (
                      <div key={block.id} style={{ borderRadius: 12, border: `2px solid ${gc}22`, overflow: 'hidden', background: 'white' }}>
                        <div style={{ background: gc, color: 'white', padding: '4px 8px', fontWeight: 800, letterSpacing: '1px', fontSize: 9, textTransform: 'uppercase' }}>⚡ {block.groupType}</div>
...
function CompactExerciseRow({ exercise, selected, latestLog, onSelect, isMobile }) {
  return (
    ...
    >
      ...
      <div style={{ width: isMobile ? 45 : 60, height: isMobile ? 45 : 60, borderRadius: 8, overflow: 'hidden', background: '#F0F5FF', border: '1px solid #E0E8F5', flexShrink: 0 }}>
        {exercise.image_url ? (
          <img src={exercise.image_url} alt={exercise.name} style={{ width: '100%', height: '100%', objectFit: 'cover' }} />
        ) : (
          <div style={{ display: 'grid', placeItems: 'center', height: '100%', fontSize: 16 }}></div>
        )}
      </div>
      
      {/* Contenu texte */}
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontWeight: 800, fontSize: isMobile ? 13 : 15, color: '#0D1B4E', marginBottom: 3, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{exercise.name}</div>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
          <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', background: '#2C64E5', color: 'white', borderRadius: 4 }}>{exercise.sets} × {exercise.reps}</span>
          <span style={{ fontSize: 10, padding: '2px 6px', background: '#EEF4FF', color: '#2C64E5', borderRadius: 4, border: '1px solid #DCE5F3' }}> {exercise.rest || '—'}</span>
        </div>
      </div>

      {/* Indicateur sélection */}
      <div style={{ color: selected ? '#2C64E5' : '#E0E0E0', fontSize: 18 }}>{selected ? '●' : '○'}</div>
    </button>
  )
}
...
function ExerciseWorkspace({ exercise, input, onInput, onLog, logging, onImageOpen, latestLog, isMobile }) {
  return (
    <div style={{ borderRadius: 10, border: '1.5px solid #2C64E5', background: '#F8FBFF', padding: isMobile ? 10 : 16, marginBottom: 8 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr', gap: 10 }}>
        <div>
          ...
          <div style={{ display: 'flex', gap: 6, marginBottom: 8 }}>
            ...
            <span style={{ fontSize: 11, padding: '3px 8px', background: '#EEF4FF', color: '#2C64E5', borderRadius: 20 }}>⏱ {exercise.rest}</span>
          </div>
        ...
        <div>
          ...
          <button type="button" onClick={onLog} disabled={logging} style={{ marginTop: 6, width: '100%', border: 'none', background: '#2C64E5', color: 'white', borderRadius: 8, padding: '8px', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>
            {logging ? '...' : '✓ Enregistrer'}
          </button>
        </div>
      </div>
...
function inputStyle() {
  return { width: '100%', boxSizing: 'border-box', padding: '8px', borderRadius: 8, border: '1px solid #C5D8F5', background: 'white', fontSize: 12, color: '#0D1B4E', fontFamily: "'DM Sans',sans-serif" }
}

function Field({ label, children }) {
  return (<div><div style={{ fontSize: 9, fontWeight: 800, color: '#6B7A99', marginBottom: 2 }}>{label}</div>{children}</div>)
}

function MiniKpi({ label, value }) {
  return <div style={{ border: '1px solid #C5D8F5', borderRadius: 10, background: '#EEF4FF', padding: 8 }}><div style={{ fontSize: 9, color: '#6B8ED6', textTransform: 'uppercase', letterSpacing: '1px', fontWeight: 700 }}>{label}</div><div style={{ fontWeight: 900, fontSize: 20, color: '#0D1B4E' }}>{value}</div></div>
}

function InfoCard({ title, value }) {
  return <div style={{ border: '1px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 10 }}><div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 4, fontSize: 9, textTransform: 'uppercase' }}>{title}</div><div style={{ color: '#4A6FB5', fontSize: 12 }}>{value}</div></div>
}