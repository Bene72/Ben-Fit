...
export default function TrainingPage() {
  ...
  return (
    ...
    >
      {error ? (
        <div style={{ marginBottom: 10 }}>
          <SurfaceCard padded style={{ borderColor: '#F3C4C4', background: '#FEF2F2' }}>
            <strong style={{ display: 'block', marginBottom: 4, color: '#B42318', fontSize: 12 }}>Erreur</strong>
            <div style={{ color: '#B42318', fontSize: 12 }}>{error}</div>
          </SurfaceCard>
        </div>
      ) : null}
      ...
      {success ? (
        <div style={{ marginBottom: 10 }}>
          <SurfaceCard padded style={{ borderColor: '#C9E9D5', background: '#F0FBF4' }}>
            <strong style={{ display: 'block', marginBottom: 4, color: '#16804A', fontSize: 12 }}>OK</strong>
            <div style={{ color: '#16804A', fontSize: 12 }}>{success}</div>
          </SurfaceCard>
        </div>
      ) : null}
      ...
      {activeTab === 'session' ? (
        <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(200px, 0.92fr) minmax(380px, 1.4fr) minmax(220px, 0.92fr)', gap: 12, alignItems: 'start' }}>
                  ...
                  return (
                    <button key={workout.id} type="button" onClick={() => openSession(workout.id)} style={{ width: '100%', textAlign: 'left', borderRadius: 10, border: active ? '1.5px solid #2C64E5' : '1px solid #DCE5F3', background: active ? '#F5F8FF' : '#FFFFFF', padding: '10px 12px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', gap: 6 }}>
                        <div>
                          <div style={{ fontWeight: 900, fontSize: 14, color: '#0D1B4E', marginBottom: 2 }}>{workout.name}</div>
                          <div style={{ color: '#6B7A99', fontSize: 11 }}>{getWorkoutDayLabel(workout.day_of_week)} · {(workout.exercises || []).length} exercice(s)</div>
                        </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {exerciseBlocks.map((block) => {
                  if (block.kind === 'group' && block.groupType === 'Workout Block') {
                    ...
                    return (
                      <div key={block.id} style={{ borderRadius: 10, overflow: 'hidden', border: `2px solid ${tc}`, boxShadow: '0 4px 20px rgba(13,27,78,0.15)', marginBottom: 2 }}>
                        <div style={{ background: tc, padding: '8px 12px', display: 'flex', alignItems: 'center', gap: 6 }}>
                          ...
                          <div>
                            ...
                            {meta.objective && <div style={{ color: 'rgba(255,255,255,0.8)', fontSize: 9, marginTop: 1 }}>🎯 {meta.objective}</div>}
                          </div>
                        </div>
                        <div style={{ background: '#0D1B4E', padding: '8px 12px' }}>
                          {block.exercises.map((e, i) => (
                            <div key={e.id} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '3px 0', borderBottom: i < block.exercises.length - 1 ? '1px solid rgba(255,255,255,0.07)' : 'none' }}>
                          ...
                          {meta.coachNote && (
                            <div style={{ marginTop: 6, padding: '5px 8px', background: 'rgba(255,255,255,0.06)', borderRadius: 6, borderLeft: `3px solid ${tc}` }}>
                              <span style={{ color: 'rgba(255,255,255,0.45)', fontSize: 9, letterSpacing: '1px', textTransform: 'uppercase' }}>🧠 Note coach </span>
                              <span style={{ color: 'rgba(255,255,255,0.85)', fontSize: 10 }}>{meta.coachNote}</span>
                            </div>
                          )}
                          {meta.rest && meta.rest !== '0s' && (
                            <div style={{ marginTop: 4, fontSize: 9, color: 'rgba(255,255,255,0.35)' }}>⏱ Repos entre rounds : {meta.rest}</div>
                          )}
                        </div>
                      </div>
                    )
                  ...
                  if (block.kind === 'group') {
                    ...
                    return (
                      <div key={block.id} style={{ borderRadius: 12, border: `2px solid ${gc}22`, overflow: 'hidden', background: 'white' }}>
                        <div style={{ background: gc, color: 'white', padding: '6px 10px', fontWeight: 800, letterSpacing: '1.2px', fontSize: 10, textTransform: 'uppercase' }}>
                          ⚡ {block.groupType}
                        </div>
                        <div style={{ padding: 8 }}>
              ...
              {selectedExercise ? (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {selectedExercise.image_url ? (
                    <button type="button" onClick={() => setImageLightbox(selectedExercise.image_url)} style={{ padding: 0, border: '1px solid #DCE5F3', borderRadius: 12, overflow: 'hidden', cursor: 'pointer', background: 'white' }}>
                  ...
                  <div style={{ border: '1px solid #DCE5F3', borderRadius: 12, background: '#FFFFFF', padding: 10 }}>
                    <div style={{ fontWeight: 900, color: '#0D1B4E', marginBottom: 6, fontSize: 11 }}>Historique rapide</div>
                    {selectedLogs.length ? (
                        ...
                        {selectedLogs.slice(0, 8).map((log) => (
                          <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 6, alignItems: 'center' }}>
                            <div>
                              ...
                              <div style={{ color: '#6B7A99', fontSize: 10 }}>{safeDateLabel(getLogDate(log))}</div>
                            </div>
                      ...
                      <div style={{ color: '#6B7A99', fontSize: 11 }}>Pas encore d'historique exploitable.</div>
                    )}
                  </div>
                </div>
              ) : (
                ...
                <div key={exerciseName} style={{ border: '1px solid #DCE5F3', borderRadius: 12, background: '#FFFFFF', padding: 10 }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    {list.slice(0, 8).map((log) => (
                      <div key={log.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 6, alignItems: 'center' }}>
                        <div>
                          <div style={{ fontWeight: 700, color: '#0D1B4E', fontSize: 11 }}>{latestPerfText(log)}</div>
                          <div style={{ color: '#6B7A99', fontSize: 10 }}>{safeDateLabel(getLogDate(log))}</div>
                        </div>
                        {getLogNote(log) ? <StatusBadge tone="accent">{getLogNote(log)}</StatusBadge> : null}
                      </div>
...
function ExerciseRow({ exercise, selected, latestLog, onSelect }) {
  return (
    ...
    >
      <div style={{ display: 'grid', gridTemplateColumns: '50px minmax(0,1fr) auto', gap: 8, alignItems: 'center' }}>
        <div style={{ width: 50, height: 50, borderRadius: 8, overflow: 'hidden', background: '#E8F0FF', border: '1.5px solid #C5D8F5', flexShrink: 0 }}>
          {exercise.image_url ? (
        ...
        <div>
          <div style={{ fontWeight: 800, fontSize: 13, color: '#0D1B4E', marginBottom: 4 }}>{exercise.name}</div>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', background: '#2C64E5', color: 'white', borderRadius: 20 }}>{exercise.sets} × {exercise.reps}</span>
            <span style={{ fontSize: 10, padding: '2px 6px', background: '#EEF4FF', color: '#2C64E5', borderRadius: 20, border: '1px solid #C5D8F5' }}>⏱ {exercise.rest || '—'}</span>
            {exercise.target_weight ? <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', background: '#FFF3EE', color: '#C45C3A', borderRadius: 20 }}>🎯 {exercise.target_weight}</span> : null}
          </div>
          {latestLog ? <div style={{ color: '#6B8ED6', marginTop: 4, fontSize: 10, fontWeight: 500 }}>🕐 Dernier log : {latestPerfText(latestLog)}</div> : null}
        </div>
...
function ExerciseWorkspace({ exercise, input, onInput, onLog, logging, onImageOpen, latestLog, isMobile }) {
  return (
    <div style={{ borderRadius: 12, border: '2px solid #2C64E5', background: '#EEF4FF', padding: 12, marginBottom: 8, boxShadow: '0 4px 20px rgba(44,100,229,0.1)' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 10 }}>
        <div style={{ fontWeight: 900, fontSize: 13, color: '#0D1B4E' }}>{exercise.name}</div>
        <span style={{ fontSize: 9, fontWeight: 700, padding: '2px 6px', background: '#2C64E5', color: 'white', borderRadius: 20 }}>Ouvert</span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(160px, 0.9fr) minmax(0, 1.1fr)', gap: 10, alignItems: 'start' }}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {exercise.image_url ? (
            <button type="button" onClick={() => onImageOpen(exercise.image_url)} style={{ padding: 0, border: '1.5px solid #C5D8F5', borderRadius: 10, overflow: 'hidden', cursor: 'pointer', background: 'white' }}>
              <img src={exercise.image_url} alt={exercise.name} style={{ width: '100%', aspectRatio: '16 / 10', objectFit: 'cover' }} />
            </button>
          ) : null}
          <div style={{ border: '1.5px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 10 }}>
            <div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 6, fontSize: 10, letterSpacing: '0.5px' }}>PRESCRIPTION</div>
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginBottom: 6 }}>
              <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 6px', background: '#2C64E5', color: 'white', borderRadius: 20 }}>{exercise.sets} × {exercise.reps}</span>
              <span style={{ fontSize: 10, padding: '2px 6px', background: '#EEF4FF', color: '#2C64E5', borderRadius: 20, border: '1px solid #C5D8F5' }}>⏱ {exercise.rest || '—'}</span>
            </div>
            <div style={{ color: '#4A6FB5', lineHeight: 1.5, fontSize: 11 }}>{exercise.note || 'Aucune note particulière.'}</div>
          </div>
        </div>

        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          <div style={{ border: '1.5px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 12 }}>
            <div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 8, fontSize: 10, letterSpacing: '0.5px' }}>MON RÉSULTAT</div>
            ...
            <div style={{ marginTop: 6 }}>
              <Field label="Commentaire">
                <textarea value={input.note || ''} onChange={(e) => onInput('note', e.target.value)} placeholder="Ressenti, difficulté, repère utile…" style={{ ...inputStyle(), minHeight: 60, resize: 'vertical' }} />
              </Field>
            </div>
            <button type="button" onClick={onLog} disabled={logging}
              style={{ marginTop: 8, width: '100%', border: 'none', background: logging ? '#7FA3E8' : '#2C64E5', color: 'white', borderRadius: 8, padding: '9px', fontSize: 12, fontWeight: 800, cursor: logging ? 'not-allowed' : 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
              {logging ? 'Enregistrement…' : '✓ Enregistrer'}
            </button>
          </div>
          {latestLog && (
            <div style={{ border: '1.5px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 10 }}>
              <div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 4, fontSize: 9, letterSpacing: '0.5px', textTransform: 'uppercase' }}>Dernière perf</div>
              <div style={{ color: '#2C64E5', fontWeight: 700, fontSize: 12 }}>{latestPerfText(latestLog)}</div>
              <div style={{ color: '#6B8ED6', fontSize: 10, marginTop: 1 }}>{safeDateLabel(getLogDate(latestLog))}{getLogNote(latestLog) ? ` · ${getLogNote(latestLog)}` : ''}</div>
            </div>
          )}
        </div>
      </div>
...
function Field({ label, children }) {
  return (
    <div>
      <div style={{ fontSize: 9, fontWeight: 800, color: '#6B7A99', marginBottom: 4 }}>{label}</div>
      {children}
    </div>
  )
}
...
function MiniKpi({ label, value }) {
  return <div style={{ border: '1.5px solid #C5D8F5', borderRadius: 10, background: '#EEF4FF', padding: 8 }}><div style={{ fontSize: 9, color: '#6B8ED6', textTransform: 'uppercase', letterSpacing: '1.1px', marginBottom: 4, fontWeight: 700 }}>{label}</div><div style={{ fontWeight: 900, fontSize: 18, color: '#0D1B4E' }}>{value}</div></div>
}

function InfoCard({ title, value }) {
  return <div style={{ border: '1.5px solid #C5D8F5', borderRadius: 10, background: 'white', padding: 10 }}><div style={{ fontWeight: 800, color: '#0D1B4E', marginBottom: 6, fontSize: 9, letterSpacing: '0.5px', textTransform: 'uppercase' }}>{title}</div><div style={{ color: '#4A6FB5', lineHeight: 1.5, fontSize: 11 }}>{value}</div></div>
}