...
export default function Dashboard() {
  ...
  const changePassword = async () => {
    ...
    const { error: signInError } = await supabase.auth.signInWithPassword({ email: user.email, password: pwdForm.current })
    if (signInError) { setPwdError('Ancien mot de passe incorrect.'); return }
    ...
    <Layout title="Dashboard" user={user}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '12px' }}>
        {[
          ...
          <button key={link.href} onClick={() => router.push(link.href)} style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '14px', padding: '20px', textAlign: 'left', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", transition: 'all 0.2s', borderTop: `3px solid ${link.color}` }}>
            ...
            <div style={{ fontSize: '12px', color: '#6B7A99' }}>{link.sub}</div>
          </button>
        ))}
      </div>
    </Layout>
