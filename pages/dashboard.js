...
function BodyTracker({ measures, weightForm, setWeightForm, editWeight, setEditWeight, saving, saveWeight, deleteMeasure, deletingId, bodyTab, setBodyTab, chartField, setChartField }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ background: 'linear-gradient(135deg, #0D1B4E, #2C4A9E)', borderRadius: 16, padding: '22px 24px', color: 'white', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        ...
        <button onClick={() => setEditWeight(!editWeight)} style={{ ...btnB(editWeight ? 'rgba(255,255,255,0.15)' : 'white', editWeight ? 'white' : '#0D1B4E'), border: editWeight ? '1px solid rgba(255,255,255,0.3)' : 'none' }}>
      ...
      {editWeight && (
        <div style={{ background: '#F0F4FF', border: '1.5px solid #C5D0F0', borderRadius: 14, padding: '20px 18px' }}>
          <div style={{ fontWeight: 800, fontSize: 14, color: '#0D1B4E', marginBottom: 16 }}>
            ✏️ {new Date().toLocaleDateString('fr-FR', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' })}
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button onClick={saveWeight} disabled={saving || !weightForm.weight} style={{ ...btnB('#0D1B4E', 'white'), opacity: !weightForm.weight ? 0.45 : 1 }}>
              {saving ? 'Sauvegarde…' : '✓ Enregistrer'}
            </button>
            <button onClick={() => setEditWeight(false)} style={btnB('transparent', '#6B7A99', '#C5D0F0')}>Annuler</button>
          </div>
        </div>
      )}
