...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  async function syncImages(forceAll = false) {
    ...
    try {
      // 1. Lister tous les fichiers du bucket exercise-images
      const { data: storageFiles } = await supabase.storage.from('exercise-images').list('', { limit: 1000 })
      const files = (storageFiles || []).filter(f => f.name && !f.name.startsWith('.'))

      if (!files.length) {
        alert('Aucun fichier dans le bucket exercise-images')
        setImageSyncing(false)
        return
      }

      // Noms de fichiers normalisés pour matching
      const normalizedFiles = files.map(f => ({
        original: f.name,
        normalized: f.name.toLowerCase().replace(/\.[^.]+$/, '').replace(/[-_]/g, ' ').replace(/\s+/g, ' ').trim(),
        url: `${SUPABASE_URL}/storage/v1/object/public/exercise-images/${encodeURIComponent(f.name)}`
      }))
      ...
      // 4. Pour les exercices sans match local → demander à l'IA
      if (noMatch.length > 0) {
        const prompt = `Tu es un assistant de matching d'images d'exercices de musculation.
Voici une liste de noms de fichiers disponibles (normalisés, sans extension) :
${fileNames.join('\n')}

Pour chaque exercice ci-dessous, trouve le fichier le plus proche ou réponds "null" si aucun ne correspond.
Réponds UNIQUEMENT en JSON valide : { "matches": { "nom_exercice": "nom_fichier_ou_null" } }

Exercices à matcher :
${noMatch.map(e => e.name).join('\n')}`
        ...
        if (jsonMatch) {
          ...
          for (const ex of noMatch) {
            const matched = matches[ex.name]
            if (matched && matched !== 'null') {
              const fileEntry = normalizedFiles.find(f => f.normalized === matched || f.original.replace(/\.[^.]+$/, '') === matched)
              if (fileEntry) {
                await supabase.from('exercises').update({ image_url: fileEntry.url }).eq('id', ex.id)
              }
            }
          }
        }
