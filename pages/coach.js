         ...
         < >
          {/* Header */}
           <div style={{ padding: '16px 32px', borderBottom: '1px solid #C5D0F0', background: '#EEF2FF', position: 'sticky', top: 0, zIndex: 50, display: 'flex', alignItems: 'center', gap: '1 6px' }} >
             <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: `hsl(${(selected.full_name?.charCodeAt(0) || 65) * 7 % 360},40%,45%)`, display: 'flex', alignItems: 'c enter', justifyContent: 'center', fontSize: '14px', color: 'white', fontWeight: '700' }} >
            ...
            {(() => {
              ...
              return exs.map(ex => {
               ...
               <div key={ex.group_id} style={{ borderRadius: '12px', marginBottom: '14px', overflow: 'hidden', border: `2px solid ${tc}`, boxShadow: '0 4px 16px rgba(0,0,0,0.12)' }} >
                ...
                rendered.add(ex.id)
                return  <ExRow key={ex.id} ex={ex} wId={workout.id} edit={isEdit} onUpdate={updateExercise} onDelete={deleteExercise} onMove={moveExercise} isFirst={exs.indexOf(ex)===0} isLast={exs.indexO f(ex)===exs.length-1} / >
              })
            })()}
 ...
 <div style={{ fontSize: '11px', color: '#8FA07A' }} >● Conversation directe </div >
      ...
      return (
         <div key={msg.id} style={{ display: 'flex', justifyContent: isCoach ? 'flex-end' : 'flex-start' }} >
           <div style={{ maxWidth: '70%' }} >
             <div style={{ padding: '10px 14px', borderRadius: '14px', fontSize: '14px', lineHeight: '1.5', background: isCoach ? '#0D1B4E' : 'white', color: isCoach ? 'white' : '#0D1B4E', bord er: isCoach ? 'none' : '1px solid #C5D0F0', borderBottomRightRadius: isCoach ? '4px' : '14px', borderBottomLeftRadius: isCoach ? '14px' : '4px', opacity: msg.id?.toString().startsW ith('temp-') ? 0.7 : 1 }} >
              {msg.content}
             </div >
  ...
  {/* Input */}
   <div style={{ padding: '12px 16px', borderTop: '1px solid #C5D0F0', display: 'flex', gap: '10px', alignItems: 'flex-end' }} >
    ...
    / >
     <button onClick={send} disabled={sending || !newMsg.trim()} style={{ padding: '10px 18px', background: !newMsg.trim() ? 'rgba(13,27,78,0.3)' : '#0D1B4E', color: 'white', border: 'n one', borderRadius: '10px', fontSize: '13px', fontWeight: '600', cursor: !newMsg.trim() ? 'not-allowed' : 'pointer', fontFamily:  "'DM Sans',sans-serif " }} >
      Envoyer
     </button >
   </div >
 </div >
