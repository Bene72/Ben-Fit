...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  async function syncImages(forceAll = false) {
    ...
    try {
      const { data: library } = await supabase.from('exercise_library').select('name, image_url').not('image_url', 'is', null)
      const lib = library || []

      for (const workout of workouts) {
        for (const ex of (workout.exercises || [])) {
          if (!forceAll && ex.image_url) continue
          const bestImage = findBestExerciseImage(ex.name, lib)
          if (bestImage && ex.image_url !== bestImage) {
            await supabase.from('exercises').update({ image_url: bestImage }).eq('id', ex.id)
          }
        }
      }
  ...
  const updateExercise = async (workoutId, exId, field, value) => {
    ...
    if (field === 'name') {
      try {
        const { data: library } = await supabase.from('exercise_library').select('name, image_url').not('image_url', 'is', null)
        const bestImage = findBestExerciseImage(value, library || [])
