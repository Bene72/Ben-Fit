...
import { duplicateProgramClientSide, insertAiProgramClientSide, normalizeWorkoutPayload } from '../lib/services/coach'
...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const duplicateProgram = async (targetClientId) => {
    ...
    setDuplicating(true)
    try {
      const payload = workouts.map(normalizeWorkoutPayload)
      try {
        await callEdgeFunction('duplicate-program', { targetClientId, workouts: payload })
      } catch (edgeError) {
        await duplicateProgramClientSide({ workouts: payload, targetClientId })
      }
  ...
  const insertAiProposal = async () => {
    ...
    try {
      try {
        await callEdgeFunction('insert-ai-program', { clientId, aiProposal })
      } catch (edgeError) {
        await insertAiProgramClientSide({ aiProposal, clientId })
      }
      const { data } = await supabase.from('workouts').select('*, exercises(*)').eq('client_id', clientId).eq('is_archived', false).order('day_of_week')
      setWorkouts((data || []).map(w => ({ ...w, exercises: (w.exercises || []).sort((a, b) => a.order_index - b.order_index) })))
      setShowAI(false)
