...
export default function CoachPage() {
  ...
  async function loadClientContext(clientId) {
    try {
      ...
      const [{ data: workoutsData, error: workoutsError }, { data: nutritionData, error: nutritionError }, { data: bilansData, error: bilansError }] =
        await Promise.all([
          supabase
            .from('workouts')
            .select('*, exercises(*)')
  ...
  const filteredClients = useMemo(() => {
    ...
    return clients.filter((client) => {
      const hay = `${client.first_name || ''} ${client.last_name || ''} ${client.email || ''}`.toLowerCase()
                      ...
                      <div className="ui-stack">
                        <div className="ui-kpi-row">
                          <div className="ui-kpi">
                            ...
                            <p className="ui-kpi-value">{nutritionPlan.calories || '—'}</p>
                          </div>
                          <div className="ui-kpi">
                            <p className="ui-kpi-label">Protéines</p>
                            <p className="ui-kpi-value">{nutritionPlan.protein || '—'}</p>
                          </div>
                          <div className="ui-kpi">
                            <p className="ui-kpi-label">Glucides</p>
                            <p className="ui-kpi-value">{nutritionPlan.carbs || '—'}</p>
                          </div>
                        </div>
