...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  // Liste des fichiers — chargée dynamiquement depuis le bucket Supabase via l'API
  // Aucune liste hardcodée : si une image est ajoutée au bucket, elle apparaît automatiquement
  const [exerciseImageFiles, setExerciseImageFiles] = useState([])
  const [exerciseFilesLoading, setExerciseFilesLoading] = useState(true)

  useEffect(() => {
    setExerciseFilesLoading(true)
    fetch('/api/exercise-images')
      .then(r => { if (!r.ok) throw new Error('API error ' + r.status); return r.json() })
      .then(d => {
        const files = (d.files || []).filter(Boolean)
        setExerciseImageFiles(files.sort((a, b) => a.localeCompare(b, 'fr')))
        if (d.warning) console.warn('[ExerciseImages]', d.warning)
      })
      .catch(err => {
        console.error('[ExerciseImages] Erreur chargement:', err.message)
        setExerciseImageFiles([])
      })
      .finally(() => setExerciseFilesLoading(false))
  }, [])

  const EXERCISE_IMAGE_FILES = exerciseImageFiles

  // Debug: afficher le nombre d'images chargées (à retirer en prod si souhaité)
  // console.log('[ExerciseImages] Chargées:', EXERCISE_IMAGE_FILES.length)
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {workouts.length > 0 && (
            <button onClick={() => syncImages(true)} disabled={imageSyncing || exerciseFilesLoading} style={btn('#8FA07A', 'white')}>
              {exerciseFilesLoading ? '⏳ Chargement images…' : imageSyncing ? '🔄 Sync…' : `🔄 Sync images (${EXERCISE_IMAGE_FILES.length})`}
            </button>
          )}
      ...
      {exPicker && (
        <ExercisePicker
          ...
          exerciseFilesLoading={exerciseFilesLoading}
...
// ─── EXERCISE PICKER MODAL ──────────────────────────────────
function ExercisePicker({ picker, query, setQuery, mode, setMode, freeVal, setFreeVal, onConfirm, onClose, exerciseFiles, exerciseFilesLoading, supabaseUrl }) {
  ...
  const filtered = (query.length < 1
    ? normalizedFiles
    : normalizedFiles.filter(f => f.name.toLowerCase().includes(query.toLowerCase()))
      ...
      <div style={{ background: 'white', borderRadius: '16px', padding: '24px', width: '420px', maxWidth: '95vw', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', maxHeight: '80vh', display: 'flex', flexDirection: 'column' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', marginBottom: '14px' }}>
          <button onClick={() => setMode('search')} style={{ flex: 1, padding: '8px', background: mode === 'search' ? '#0D1B4E' : '#EEF2FF', color: mode === 'search' ? 'white' : '#0D1B4E', border: 'none', borderRadius: '8px', fontWeight: '600', fontSize: '13px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
            🔍 Exercices répertoriés {exerciseFiles.length > 0 ? `(${exerciseFiles.length})` : ''}
          </button>
        ...
        {mode === 'search' && (
          <>
            ...
            <div style={{ overflowY: 'auto', flex: 1, display: 'flex', flexDirection: 'column', gap: '4px' }}>
              {exerciseFilesLoading && (
                <div style={{ color: '#6B7A99', fontSize: '13px', textAlign: 'center', padding: '20px' }}>
                  ⏳ Chargement de la bibliothèque d'exercices…
                </div>
              )}
              {!exerciseFilesLoading && filtered.length === 0 && (
                <div style={{ color: '#6B7A99', fontSize: '13px', textAlign: 'center', padding: '20px' }}>
                  {exerciseFiles.length === 0
                    ? '⚠️ Aucune image trouvée dans le bucket. Vérifie SUPABASE_SERVICE_ROLE_KEY dans Vercel.'
                    : 'Aucun résultat — utilise le mode "Exercice libre"'}
                </div>
              )}
              {!exerciseFilesLoading && filtered.map(f => (
