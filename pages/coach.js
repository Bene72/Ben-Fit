...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const duplicateProgram = async (targetClientId) => {
    ...
    setDuplicating(true)
    try {
      // 1. Recharger les workouts depuis la BDD pour garantir qu'on a bien tous les exercices
      ...
      if (!freshWorkouts?.length) {
        alert('Aucune séance à dupliquer.')
      ...
      for (const workout of freshWorkouts) {
        ...
        // 3. Régénérer les group_id pour éviter les conflits chez le client cible
        const groupIdMap = {}
        exs.forEach(ex => {
          if (ex.group_id && !groupIdMap[ex.group_id]) {
            groupIdMap[ex.group_id] = 'grp_' + Date.now() + '_' + Math.random().toString(36).slice(2, 7)
          }
        })

        // 4. Insérer les exercices avec les nouveaux group_id
