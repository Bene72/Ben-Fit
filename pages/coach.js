  ...
  function ExerciseCard({ ex, wId, edit }) {
    ...
    return (
      <>
        ...
        {showImgModal && ex.image_url && (
          <div onClick={() => setShowImgModal(false)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ position: 'relative', maxWidth: '500px', width: '90%' }}>
              ...
              <button onClick={() => setShowImgModal(false)} style={{ position: 'absolute', top: '-12px', right: '-12px', width: '32px', height: '32px', borderRadius: '50%', background: 'white', border: 'none', fontSize: '18px', cursor: 'pointer' }}>×</button>
            </div>
          </div>
        )}

        <div style={{ background: 'white', borderRadius: '10px', border: '1px solid #E8ECFA', marginBottom: '10px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'stretch' }}>
            ...
            <div style={{ width: '100px', flexShrink: 0, background: '#EEF2FF', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: ex.image_url ? 'pointer' : 'default' }}
              onClick={() => ex.image_url && setShowImgModal(true)}>
              {ex.image_url
            ...
            <div style={{ flex: 1, padding: '12px 14px' }}>
              {edit
                ? <input value={ex.name} onChange={e => onUpdate(wId, ex.id, 'name', e.target.value)} style={{ ...inp, fontWeight: '600', fontSize: '14px', marginBottom: '8px' }} />
              ...
              {edit ? (
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: '6px', marginBottom: '6px' }}>
                  <div>
                    ...
                    <input type="number" value={ex.sets} onChange={e => onUpdate(wId, ex.id, 'sets', e.target.value)} style={{ ...inp, textAlign: 'center', padding: '4px 6px' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', color: '#999', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '2px' }}>Reps</div>
                    <input value={ex.reps} onChange={e => onUpdate(wId, ex.id, 'reps', e.target.value)} style={{ ...inp, textAlign: 'center', padding: '4px 6px' }} />
                  </div>
                  <div>
                    <div style={{ fontSize: '9px', color: '#999', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: '2px' }}>Repos</div>
                    <select value={ex.rest||'90s'} onChange={e => onUpdate(wId, ex.id, 'rest', e.target.value)} style={{ ...inp, textAlign: 'center', padding: '4px 6px' }}>
                  ...
                  <div>
                    ...
                    <input value={ex.target_weight||''} onChange={e => onUpdate(wId, ex.id, 'target_weight', e.target.value)} placeholder="80 kg" style={{ ...inp, textAlign: 'center', padding: '4px 6px' }} />
                  </div>
                </div>
              ) : (
                <div style={{ display: 'flex', gap: '6px', flexWrap: 'wrap', marginBottom: '6px' }}>
                  ...
                  {lastLog && <span style={{ fontSize: '11px', padding: '3px 8px', background: 'rgba(74,111,212,0.08)', color: '#4A6FD4', borderRadius: '20px' }}>🕐 {lastLog.weight_used || '—'}</span>}
                </div>
              )}

              {edit
                ? <input value={ex.note||''} onChange={e => onUpdate(wId, ex.id, 'note', e.target.value)} placeholder="Consigne coach…" style={{ ...inp, fontSize: '12px', padding: '4px 8px' }} />
          ...
          {!edit && showLog && (
            <div style={{ padding: '10px 14px 12px', background: '#F5F8FF', borderTop: '1px solid #E8ECFA' }}>
              ...
              <div style={{ display: 'flex', gap: '8px', alignItems: 'center', flexWrap: 'wrap' }}>
                <input value={logInputs[ex.id]?.weight || ''} onChange={e => onLogInput(ex.id, 'weight', e.target.value)} placeholder="Charge (80 kg)" style={{ ...inp, width: '130px' }} />
                <input value={logInputs[ex.id]?.reps || ''} onChange={e => onLogInput(ex.id, 'reps', e.target.value)} placeholder="Reps réelles" style={{ ...inp, width: '110px' }} />
                <input value={logInputs[ex.id]?.note || ''} onChange={e => onLogInput(ex.id, 'note', e.target.value)} placeholder="Note / ressenti" style={{ ...inp, flex: 1, minWidth: '120px' }} />
                <button onClick={() => logPerformance(ex)} disabled={logging[ex.id]} style={{ padding: '6px 14px', background: '#0D1B4E', color: 'white', border: 'none', borderRadius: '7px', fontSize: '12px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif", whiteSpace: 'nowrap' }}>
                  {logging[ex.id] ? '…' : '✓ Logger'}
                </button>
              </div>
            </div>
          )}
            ...
            {isOpen && (
              <div style={{ padding: '18px 20px' }}>
                <div style={{ marginBottom: '14px', display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '18px', color: '#0D1B4E', letterSpacing: '1px' }}>{workout.name}</div>
                  <span style={{ fontSize: '12px', color: '#6B7A99' }}>{workout.exercises?.length || 0} exercices · {workout.duration_min} min</span>
                </div>
