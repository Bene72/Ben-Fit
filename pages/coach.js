            ...
            {clients.map(client => {
              ...
              return (
                <button key={client.id} onClick={() => selectClient(client)} style={{ display: 'flex', alignItems: 'center', gap: '10px', padding: '10px 12px', borderRadius: '8px', cursor: 'pointer', background: isSelected ? '#0D1B4E' : 'transparent', border: 'none', width: '100%', textAlign: 'left', fontFamily: "'DM Sans',sans-serif", marginBottom: '2px', transition: 'all 0.2s' }}>
                  ...
                  <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: w >= 4 ? '#8FA07A' : w <= 1 ? '#C45C3A' : '#4A6FD4', flexShrink: 0 }} />
                </button>
              )
            })}
          </div>
          ...
          {showNewClient && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ background: '#0D1B4E', border: '1px solid #3E3E30', borderRadius: '16px', padding: '28px', width: '380px', fontFamily: "'DM Sans',sans-serif" }}>
                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: '18px', color: '#EEF2FF', marginBottom: '20px' }}>Nouveau client</div>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input value={newClient.full_name} onChange={e => setNewClient(p => ({ ...p, full_name: e.target.value }))} placeholder="Jean Dupont" style={{ width: '100%', padding: '9px 12px', border: '1px solid #3E3E30', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: '#2A2A20', color: '#EEF2FF', outline: 'none' }} />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  ...
                  <input type="email" value={newClient.email} onChange={e => setNewClient(p => ({ ...p, email: e.target.value }))} placeholder="jean@email.com" style={{ width: '100%', padding: '9px 12px', border: '1px solid #3E3E30', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: '#2A2A20', color: '#EEF2FF', outline: 'none' }} />
                </div>
                <div style={{ marginBottom: '16px' }}>
                  ...
                  <input value={newClient.password} onChange={e => setNewClient(p => ({ ...p, password: e.target.value }))} placeholder="MotDePasse123!" style={{ width: '100%', padding: '9px 12px', border: '1px solid #3E3E30', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: '#2A2A20', color: '#EEF2FF', outline: 'none' }} />
                </div>
                {createError && <div style={{ color: '#C45C3A', fontSize: '13px', marginBottom: '12px' }}>{createError}</div>}
                <div style={{ display: 'flex', gap: '8px' }}>
                  <button onClick={createClient} disabled={creating} style={{ flex: 1, padding: '10px', background: '#4A6FD4', color: 'white', border: 'none', borderRadius: '8px', fontSize: '13px', fontWeight: '600', cursor: creating ? 'not-allowed' : 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
                    {creating ? 'Création…' : '✓ Créer le client'}
                  </button>
                  <button onClick={() => { setShowNewClient(false); setCreateError('') }} style={{ padding: '10px 16px', background: 'transparent', color: '#6B7A99', border: '1px solid #3E3E30', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>Annuler</button>
                </div>
              </div>
            </div>
          )}
          ...
          {!selected ? (
            ...
            <>
              ...
              <div style={{ padding: '28px 32px' }}>
                ...
                {tab === 'nutrition' && selected && (<><NutritionTab clientId={selected.id} clientName={selected.full_name} /><div style={{marginTop:'24px', padding:'16px 20px', background:'#EEF2FF', borderRadius:'12px', border:'1px solid #C0CAEF', display:'flex', justifyContent:'space-between', alignItems:'center'}}><div><div style={{fontWeight:'700', color:'#0D1B4E', fontSize:'14px'}}>📊 Dashboard nutrition complet</div><div style={{fontSize:'12px', color:'#666', marginTop:'3px'}}>Voir anneaux, score, graphique et détail aliments</div></div><button onClick={()=>router.push('/coach/'+selected.id)} style={{padding:'8px 18px', background:'#0D1B4E', color:'white', border:'none', borderRadius:'8px', fontSize:'13px', fontWeight:'600', cursor:'pointer'}}>Ouvrir →</button></div></>)}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: '16px', marginBottom: '20px' }}>
        ...
        <div style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '14px', padding: '20px 24px', borderTop: '3px solid #4A6FD4' }}>
          ...
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: '28px', fontWeight: '700' }}>{sessions}<span style={{ fontSize: '16px', color: '#6B7A99' }}>/{target}</span></div>
          {editStats && (
        ...
        <div style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '14px', padding: '20px 24px', borderTop: '3px solid #C45C3A' }}>
          ...
          <div style={{ fontFamily: "'Playfair Display',serif", fontSize: '28px', fontWeight: '700' }}>{currentWeight}<span style={{ fontSize: '16px', color: '#6B7A99' }}> kg</span></div>
          {editStats && (
            <div style={{ marginTop: '8px' }}>
  ...
  return (
    <div style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '14px', display: 'flex', flexDirection: 'column', height: '500px' }}>
      <div style={{ flex: 1, padding: '20px', overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: '10px' }}>
        {messages.length === 0 && <div style={{ textAlign: 'center', color: '#6B7A99', fontSize: '14px', margin: 'auto' }}>Aucun message avec {clientName?.split(' ')[0]}</div>}
        {messages.map(msg => {
          const isCoach = msg.sender_id === coachId
          return (
            <div key={msg.id} style={{ display: 'flex', justifyContent: isCoach ? 'flex-end' : 'flex-start' }}>
              <div style={{ maxWidth: '70%', padding: '10px 14px', borderRadius: '14px', fontSize: '14px', lineHeight: '1.5', background: isCoach ? '#0D1B4E' : '#C5D0F0', color: isCoach ? 'white' : '#0D1B4E', borderBottomRightRadius: isCoach ? '4px' : '14px', borderBottomLeftRadius: isCoach ? '14px' : '4px' }}>
                {msg.content}
              </div>
            </div>
          )
      ...
      <div style={{ padding: '12px 16px', borderTop: '1px solid #C5D0F0', display: 'flex', gap: '10px' }}>
        <input value={newMsg} onChange={e => setNewMsg(e.target.value)} onKeyDown={e => e.key === 'Enter' && send()} placeholder={`Message à ${clientName?.split(' ')[0]}…`} style={{ flex: 1, padding: '10px 14px', border: '1.5px solid #C5D0F0', borderRadius: '10px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: 'white', outline: 'none' }} />
        <button onClick={send} style={btn('#0D1B4E', 'white')}>Envoyer</button>
      </div>
    </div>
  )
}
