...
function NutritionTab({ clientId, clientName }) {
  ...
  return (
    <div>
      {/* Plan objectifs */}
      <div style={{ background: '#FDFAF4', border: '1px solid #E0D9CC', borderRadius: '14px', padding: '20px 24px', marginBottom: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
        ...
        {editPlan ? (
          ...
          <div style={{ color: '#7A7A6A', fontSize: '14px', textAlign: 'center', padding: '10px' }}>Aucun plan nutritionnel. Clique sur "+ Créer le plan" pour commencer.</div>
        )}
      </div>
    ...
    <div style={{ textAlign:'center' }}>
      <div style={{ position:'relative', display:'inline-block' }}>
        ...
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', textAlign:'center', lineHeight:1.2 }}>
          <div style={{ fontWeight:'800', fontSize:'14px', color: over?'#C45C3A':'#1A1A14' }}>{value}</div>
...
function NutritionScoreBlock({ log, plan }) {
  ...
  const color = rounded >= 80 ? '#5A8A5A' : rounded >= 50 ? '#A07820' : '#C45C3A'
  const feedback = []
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'12px', marginBottom:'16px' }}>
      ...
      <div style={{ padding:'14px 18px', borderRadius:'12px', background:'#FFF8E6', border:'1px solid #F0E0A0' }}>
        <div style={{ fontWeight:'700', fontSize:'12px', color:'#7A5A00', marginBottom:'6px' }}>Feedback</div>
...
function NutritionFoodBlock({ log, clientId }) {
  ...
  return (
    <div style={{ background:'white', borderRadius:'12px', border:'1px solid #EAEAEA', overflow:'hidden' }}>
      ...
      {showSearch && log && (
        <div style={{ padding:'12px 16px', background:'#F8F9FC', borderBottom:'1px solid #EAEAEA' }}>
      ...
      {sortedWeeks.map(weekStart => {
        ...
        return (
          <div key={weekStart} style={{ background:'white', borderRadius:'12px', border:`1px solid ${isCurrent?'#C0CAEF':'#EAEAEA'}`, overflow:'hidden', boxShadow:'0 2px 6px rgba(0,0,0,0.05)' }}>
            <div style={{ padding:'10px 16px', background:isCurrent?'#EEF2FF':'#F8F8F8', borderBottom:'1px solid #EAEAEA', display:'flex', justifyContent:'space-between' }}>
            ...
            <div style={{ display:'grid', gridTemplateColumns:'130px 1fr 1fr 1fr 1fr', background:'#FAFAFA', borderBottom:'1px solid #F0F0F0' }}>
                ...
                <div key={date}>
                  <div onClick={() => !isFuture && setOpenDay(isOpen?null:date)}
                    ...
                    <div style={{ padding:'9px 12px' }}>
                      <div style={{ fontSize:'12px', fontWeight:isToday?'700':'500', color:isFuture?'#CCC':'#1A1A14' }}>{isToday?'📍 ':''}{dayName}</div>
                    </div>
                    {macros.map(m => {
                  ...
                  {isOpen && (
                    <div style={{ padding:'14px 16px', background:'#F8F9FC', borderBottom:'2px solid #E8ECFA' }}>
