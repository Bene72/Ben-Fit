            ...
            {isOpen && (
              <div style={{ padding: '16px 20px' }}>
                ...
                {workout.exercises?.length > 0 && (
                  <div style={{ display: 'grid', gridTemplateColumns: isEdit ? '24px 2fr 40px 45px 55px 55px 3fr 28px' : '1fr 60px 70px 80px 90px 1fr', gap: '6px', padding: '8px 14px', marginBottom: '4px' }}>
...
function ExRow({ ex, wId, edit, onUpdate, onDelete, onMove, isFirst, isLast }) {
  ...
  return (
    ...
    <div style={{ display: 'grid', gridTemplateColumns: edit ? '24px 2fr 40px 45px 55px 55px 3fr 28px' : '1fr 60px 70px 80px 90px 1fr', gap: '6px', alignItems: 'center', padding: '12px 14px', borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
      {edit && (
      ...
      {edit ? <input value={ex.name} onChange={e => onUpdate(wId, ex.id, 'name', e.target.value)} style={{ ...ci, fontWeight: '600', fontSize: '15px' }} />
        : <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            {ex.image_url
      ...
      {edit ? <input type="number" value={ex.sets} onChange={e => onUpdate(wId, ex.id, 'sets', e.target.value)} style={{ ...ci, textAlign: 'center', fontSize: '13px', padding: '10px 4px' }} />
        : <div style={{ fontSize: '13px', textAlign: 'center' }}>{ex.sets}</div>}
      {edit ? <input value={ex.reps} onChange={e => onUpdate(wId, ex.id, 'reps', e.target.value)} style={{ ...ci, textAlign: 'center', fontSize: '13px', padding: '10px 4px' }} />
        : <div style={{ fontSize: '13px', textAlign: 'center' }}>{ex.reps}</div>}
      {edit ? <select value={ex.rest||'90s'} onChange={e => onUpdate(wId, ex.id, 'rest', e.target.value)} style={{ ...ci, textAlign: 'center', fontSize: '12px', padding: '10px 2px' }}>
      ...
      {edit ? <input value={ex.target_weight||''} onChange={e => onUpdate(wId, ex.id, 'target_weight', e.target.value)} placeholder="80kg" style={{ ...ci, textAlign: 'center', fontSize: '13px', padding: '10px 4px' }} />
        : <div style={{ fontSize: '12px', textAlign: 'center', color: '#6B7A99' }}>{ex.target_weight ? `${ex.target_weight} kg` : '—'}</div>}
      {edit ? <textarea value={ex.note||''} onChange={e => onUpdate(wId, ex.id, 'note', e.target.value)} placeholder="Consigne coach…" rows={3} style={{ ...ci, resize: 'vertical', minHeight: '72px', lineHeight: '1.5' }} />
