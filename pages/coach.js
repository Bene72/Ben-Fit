...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const insertAiProposal = async () => {
    ...
    try {
      ...
      setWorkouts((data || []).map(w => ({ ...w, exercises: (w.exercises || []).sort((a, b) => (a.order_index || 0) - (b.order_index || 0)) })))
      setShowAI(false)
      setAiProposal(null)
  ...
  const updateWorkoutDay = async (workoutId, newDay) => {
    ...
    setWorkouts(prev => prev.map(w => w.id === workoutId ? { ...w, day_of_week: +newDay } : w).sort((a,b)=>(a.day_of_week||0)-(b.day_of_week||0)))
  }
  ...
  function ExerciseCard({ ex, wId, edit }) {
    ...
    return (
      <div style={{ marginBottom: '12px' }}>
        ...
        >
          <div style={{ flex: 1 }}>
            <div style={{ fontWeight: 700, fontSize: '16px', color: '#0D1B4E', marginBottom: '6px' }}>{ex.name}</div>
            <div style={{ fontSize: '13px', color: '#6B7A99' }}>
              {[ex.sets && `${ex.sets} séries`, ex.reps && `${ex.reps} reps`, ex.rest && ex.rest].filter(Boolean).join(' · ') || 'Paramètres à compléter'}
            </div>
          </div>
          <span style={{ fontSize: '12px', fontWeight: 700, padding: '8px 14px', borderRadius: '999px', background: isOpen ? '#DCE8FF' : '#EEF2FF', color: '#2563EB' }}>
            {isOpen ? 'Ouvert' : 'Détail'}
          </span>
        </div>

        {isOpen && (
          <div style={{ marginTop: '10px', border: '1px solid #4A6FD4', borderRadius: '18px', padding: '18px', background: '#FBFCFF' }}>
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.15fr 0.85fr', gap: '16px' }}>
              <div>
                <div style={{ background: '#F4F7FD', border: '1px solid #E8ECFA', borderRadius: '14px', padding: '18px', marginBottom: '14px' }}>
                  {edit ? (
                    <>
                      <label style={lbl}>Nom</label>
                      <input value={ex.name} onChange={e => onUpdate(wId, ex.id, 'name', e.target.value)} style={{ ...inp, marginBottom: '10px' }} />
                      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: '10px' }}>
                        <div><label style={lbl}>Séries</label><input type="number" value={ex.sets} onChange={e => onUpdate(wId, ex.id, 'sets', e.target.value)} style={inp} /></div>
                        <div><label style={lbl}>Reps</label><input value={ex.reps} onChange={e => onUpdate(wId, ex.id, 'reps', e.target.value)} style={inp} /></div>
                        <div><label style={lbl}>Repos</label><select value={ex.rest||'90s'} onChange={e => onUpdate(wId, ex.id, 'rest', e.target.value)} style={inp}>{['30s','45s','60s','90s','2 min','3 min','4 min','5 min'].map(r => <option key={r}>{r}</option>)}</select></div>
                        <div><label style={lbl}>Charge cible</label><input value={ex.target_weight||''} onChange={e => onUpdate(wId, ex.id, 'target_weight', e.target.value)} style={inp} placeholder="80 kg" /></div>
                      </div>
                    </>
                  ) : (
                    <>
                      <div style={{ fontWeight: 700, fontSize: '16px', color: '#0D1B4E', marginBottom: '10px' }}>{ex.name}</div>
                      <div style={{ color: '#6B7A99', lineHeight: '1.7' }}>
                        {[ex.sets && `${ex.sets} séries`, ex.reps && `${ex.reps} reps`, ex.rest && ex.rest].filter(Boolean).join(' · ')}
                ...
                <div style={{ marginBottom: '14px' }}>
                  <div style={{ fontSize: '12px', letterSpacing: '1px', textTransform: 'uppercase', color: '#6B7A99', fontWeight: '600', marginBottom: '8px' }}>Notes du coach</div>
                  {edit ? (
                    <textarea value={ex.note || ''} onChange={e => onUpdate(wId, ex.id, 'note', e.target.value)} rows={3} placeholder="Consignes, technique, RIR..." style={{ ...inp, minHeight: '96px', resize: 'vertical' }} />
                  ) : (
                    <div style={{ background: 'white', border: '1px solid #E8ECFA', borderRadius: '14px', padding: '16px', color: '#6B7A99', lineHeight: '1.7' }}>
                      {ex.note || 'Aucune note particulière pour cet exercice.'}
                    </div>
                  )}
                </div>

                <div>
                  <div style={{ fontSize: '12px', letterSpacing: '1px', textTransform: 'uppercase', color: '#6B7A99', fontWeight: '600', marginBottom: '8px' }}>Logs / résultats client</div>
                  {lastLog && (
                    <div style={{ fontSize: '12px', color: '#6B7A99', marginBottom: '8px', padding: '8px 10px', background: 'rgba(74,111,212,0.08)', borderRadius: '8px', display: 'flex', gap: '10px', flexWrap: 'wrap' }}>
                      ...
                      {lastLog.logged_at && <span style={{ color: '#9AA' }}>{new Date(lastLog.logged_at).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}</span>}
                    </div>
                  )}

                  <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '130px 110px minmax(0,1fr) auto', gap: '8px', alignItems: 'center' }}>
                    <input value={logInputs[ex.id]?.weight || ''} onChange={e => onLogInput(ex.id, 'weight', e.target.value)} placeholder="Charge (80 kg)" style={inp} />
                    <input value={logInputs[ex.id]?.reps || ''} onChange={e => onLogInput(ex.id, 'reps', e.target.value)} placeholder="Reps" style={inp} />
                    <input value={logInputs[ex.id]?.note || ''} onChange={e => onLogInput(ex.id, 'note', e.target.value)} placeholder="Note / ressenti" style={inp} />
                    <button onClick={() => logPerformance(ex)} disabled={logging[ex.id]} style={btn('#0D1B4E', 'white')}>
                      {logging[ex.id] ? '…' : 'Enregistrer'}
                    </button>
                  </div>
                  ...
                  {logs.length > 1 && (
                    <div style={{ display: 'flex', gap: '6px', marginTop: '10px', flexWrap: 'wrap' }}>
                      {logs.slice(0, 5).map((log, i) => (
                        <div key={log.id} style={{ fontSize: '10px', padding: '4px 8px', background: i === 0 ? '#0D1B4E' : '#EEF2FF', color: i === 0 ? 'white' : '#6B7A99', borderRadius: '20px' }}>
                          {log.weight_used || '—'} · {log.logged_at ? new Date(log.logged_at).toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' }) : '—'}
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div>
                {ex.image_url ? (
                  <div style={{ borderRadius: '16px', overflow: 'hidden', border: '1px solid #E8ECFA', background: '#EEF2FF', marginBottom: '14px' }}>
                    <img src={ex.image_url} alt={ex.name} style={{ width: '100%', display: 'block', objectFit: 'cover', maxHeight: '320px' }} />
                  </div>
                ) : (
                  <div style={{ borderRadius: '16px', border: '1px dashed #C5D0F0', background: '#F8FAFF', minHeight: '220px', display: 'grid', placeItems: 'center', color: '#9AA', marginBottom: '14px' }}>
                    Aucune image
                  </div>
                )}
                ...
                {edit && (
                  <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                    <button onClick={() => onDelete(wId, ex.id)} style={{ ...btn('rgba(196,92,58,0.1)', '#C45C3A'), width: '100%' }}>🗑 Supprimer cet exercice</button>
                  </div>
                )}
              </div>
            </div>
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px', gap: '12px', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          <button onClick={generateCycle} style={btn('#4A6FD4', 'white')}>🤖 Générer un cycle</button>
      ...
      {workouts.map(workout => {
        ...
        const workoutLogCount = orderedExercises.reduce((sum, ex) => sum + ((workoutLogs[ex.name] || []).length > 0 ? 1 : 0), 0)

        return (
          <div key={workout.id} style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '16px', overflow: 'hidden', marginBottom: '12px' }}>
            <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', borderBottom: isOpen ? '1px solid #D9E2F4' : 'none' }}>
              <div onClick={() => setOpenWorkout(isOpen ? null : workout.id)} style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', flex: 1 }}>
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                <select value={workout.day_of_week || 1} onChange={e => { e.stopPropagation(); updateWorkoutDay(workout.id, e.target.value) }} style={{ ...inp, width: '110px', padding: '8px 10px' }}>
            ...
            {isOpen && (
              <div style={{ padding: '18px 20px' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'stretch' : 'center', flexDirection: isMobile ? 'column' : 'row', gap: '12px', marginBottom: '16px' }}>
                  <div>
                    <div style={{ fontWeight: '800', fontSize: '20px', color: '#0D1B4E' }}>{workout.name}</div>
                    <div style={{ color: '#6B7A99', marginTop: '4px' }}>
                      Même interface que le client, mais ici tu peux programmer et logger.
                    </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,minmax(0,1fr))', gap: '10px', minWidth: isMobile ? '100%' : '280px' }}>
                    {[
                      ['Exos', workout.exercises?.length || 0],
                      ['Logs', workoutLogCount],
                      ['État', isEdit ? 'Edition' : 'Ouvert']
                    ].map(([label, value]) => (
                      <div key={label} style={{ background: '#F7FAFF', border: '1px solid #D7E0F0', borderRadius: '14px', padding: '12px' }}>
                        <div style={{ fontSize: '11px', textTransform: 'uppercase', letterSpacing: '1px', color: '#6B7A99', marginBottom: '8px' }}>{label}</div>
                        <div style={{ fontWeight: '800', fontSize: label === 'État' ? '18px' : '22px', color: '#0D1B4E' }}>{value}</div>
                      </div>
                    ))}
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {orderedExercises.map(ex => <ExerciseCard key={ex.id} ex={ex} wId={workout.id} edit={isEdit} />)}
                </div>

                {orderedExercises.length === 0 && !isEdit && (
      ...
      {showHistory && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', width: 'min(960px, 100%)', maxHeight: '85vh', overflow: 'auto', borderRadius: '18px', padding: '24px' }}>
            ...
            {archivedWorkouts.length ? archivedWorkouts.map(w => (
              <div key={w.id} style={{ border: '1px solid #E8ECFA', borderRadius: '12px', padding: '14px', marginBottom: '10px', background: '#F9FBFF' }}>
                <div style={{ fontWeight: '700', color: '#0D1B4E', marginBottom: '6px' }}>
                  {w.cycle_name || 'Cycle archivé'} · {w.name}
                </div>
                <div style={{ color: '#6B7A99', fontSize: '13px' }}>
                  {(w.exercises || []).map(ex => ex.name).filter(Boolean).join(' · ') || 'Aucun exercice'}
                </div>
              </div>
            )) : <div style={{ color: '#6B7A99' }}>Aucun historique.</div>}
          </div>
        </div>
      )}
      ...
      {showAI && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '20px' }}>
          <div style={{ background: 'white', width: 'min(980px, 100%)', maxHeight: '85vh', overflow: 'auto', borderRadius: '18px', padding: '24px' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '16px' }}>
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '24px', color: '#0D1B4E', letterSpacing: '2px' }}>Génération IA</div>
              <button onClick={() => setShowAI(false)} style={btn('transparent', '#6B7A99', '#C5D0F0')}>Fermer</button>
            </div>
            {aiLoading ? <div style={{ color: '#6B7A99' }}>Génération en cours…</div> : null}
            {aiError ? <div style={{ color: '#C45C3A', marginBottom: '12px' }}>{aiError}</div> : null}
            {aiProposal ? (
              <>
                <div style={{ marginBottom: '12px', color: '#0D1B4E', fontWeight: '700' }}>{aiProposal.cycle_name}</div>
                <div style={{ marginBottom: '18px', color: '#6B7A99' }}>{aiProposal.reasoning}</div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  {(aiProposal.workouts || []).map((w, idx) => (
                    <div key={idx} style={{ border: '1px solid #E8ECFA', borderRadius: '12px', padding: '14px', background: '#F9FBFF' }}>
                      <div style={{ fontWeight: '700', color: '#0D1B4E' }}>{w.name}</div>
                ...
                <div style={{ display: 'flex', gap: '8px', marginTop: '16px' }}>
                  <button onClick={insertAiProposal} disabled={inserting} style={btn('#0D1B4E', 'white')}>{inserting ? 'Insertion…' : 'Insérer ce cycle'}</button>
                </div>
              </>
            ) : null}
          </div>
