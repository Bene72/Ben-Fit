...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const confirmAddExercise = async (name, imageUrl) => {
    ...
    // Tente avec image_url, fallback sans si colonne absente
    let insertRes = await supabase.from('exercises').insert({ ...payload, image_url: imageUrl || null }).select().single()
    if (insertRes.error?.message?.includes('image_url')) {
      insertRes = await supabase.from('exercises').insert(payload).select().single()
    }
  ...
  const exportPDF = async () => {
    ...
    try {
      const imgToBase64 = async (url) => {
        try { const r = await fetch(url); const b = await r.blob(); return await new Promise(res => { const rd = new FileReader(); rd.onloadend = () => res(rd.result); rd.readAsDataURL(b) }) } catch { return null }
      }
      const allEx = workouts.flatMap(w => w.exercises || [])
      const ic = {}
      await Promise.all(allEx.filter(e => e.image_url).map(async e => { if (!ic[e.image_url]) ic[e.image_url] = await imgToBase64(e.image_url) }))
      const dayNames = ['Lundi','Mardi','Mercredi','Jeudi','Vendredi','Samedi','Dimanche']
      const blocks = workouts.map(workout => {
        const exHtml = (workout.exercises || []).map((ex, idx) => {
          const img = ex.image_url && ic[ex.image_url] ? `<img src="${ic[ex.image_url]}" style="width:64px;height:64px;object-fit:cover;border-radius:7px;flex-shrink:0;border:1px solid #e0e6f0;" />` : `<div style="width:64px;height:64px;border-radius:7px;background:#EEF2FF;display:flex;align-items:center;justify-content:center;font-size:22px;flex-shrink:0;">💪</div>`
          const badges = [ex.sets&&ex.reps?`<span style="background:#0D1B4E;color:white;border-radius:20px;padding:2px 9px;font-size:11px;font-weight:800;">${ex.sets} × ${ex.reps}</span>`:'', ex.rest?`<span style="background:#EEF2FF;color:#4A6FD4;border-radius:20px;padding:2px 9px;font-size:11px;font-weight:700;">⏱ ${ex.rest}</span>`:'', ex.target_weight?`<span style="background:#EEF2FF;color:#4A6FD4;border-radius:20px;padding:2px 9px;font-size:11px;font-weight:700;">🏋️ ${ex.target_weight}</span>`:''].filter(Boolean).join(' ')
          return `<div style="display:flex;gap:12px;align-items:flex-start;padding:10px 0;border-bottom:1px solid #F0F4FF;">${img}<div style="flex:1;"><div style="font-weight:800;font-size:13px;color:#0D1B4E;margin-bottom:4px;">${idx+1}. ${ex.name||'—'}</div><div style="margin-bottom:5px;">${badges}</div>${ex.note?`<div style="font-size:11px;color:#6B7A99;background:#F8FAFF;border-left:3px solid #4A6FD4;padding:5px 9px;border-radius:0 5px 5px 0;">${ex.note}</div>`:''}</div></div>`
        }).join('')
        return `<div style="page-break-inside:avoid;margin-bottom:24px;background:white;border-radius:12px;border:1px solid #E0E6F5;overflow:hidden;"><div style="background:linear-gradient(135deg,#0D1B4E,#2C4A9E);padding:12px 18px;"><div style="font-size:10px;color:rgba(255,255,255,0.55);text-transform:uppercase;letter-spacing:1.5px;">${dayNames[(workout.day_of_week||1)-1]}</div><div style="font-size:17px;font-weight:900;color:white;">${workout.name}</div><div style="font-size:11px;color:rgba(255,255,255,0.5);margin-top:2px;">${(workout.exercises||[]).length} exercices${workout.duration_min?' · '+workout.duration_min+' min':''}</div></div><div style="padding:2px 18px 8px;">${exHtml}</div></div>`
      }).join('')
      const html = `<!DOCTYPE html><html lang="fr"><head><meta charset="UTF-8"><title>Programme ${clientName}</title><style>*{box-sizing:border-box;margin:0;padding:0;}body{font-family:-apple-system,sans-serif;background:#F5F7FF;}.page{max-width:780px;margin:0 auto;padding:28px 24px;}@media print{body{background:white;}.page{padding:12px;}@page{margin:1.5cm;size:A4;}}</style></head><body><div class="page"><div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px;padding-bottom:18px;border-bottom:2px solid #E0E6F5;"><div><div style="font-size:10px;text-transform:uppercase;letter-spacing:2px;color:#9BA8C0;margin-bottom:3px;">Programme</div><div style="font-size:28px;font-weight:900;color:#0D1B4E;">${clientName}</div><div style="font-size:12px;color:#6B7A99;margin-top:4px;">Exporté le ${new Date().toLocaleDateString('fr-FR')} · ${workouts.length} séance${workouts.length>1?'s':''}</div></div><div style="text-align:right;"><div style="font-size:18px;font-weight:900;color:#0D1B4E;letter-spacing:1px;">BEN&FIT</div><div style="font-size:9px;color:#9BA8C0;letter-spacing:2px;text-transform:uppercase;">Only Benefit</div></div></div>${blocks}<div style="margin-top:20px;padding-top:14px;border-top:1px solid #E0E6F5;text-align:center;font-size:10px;color:#C5D0F0;">BEN&FIT · Programme de ${clientName}</div></div><script>window.onload=()=>window.print()</script></body></html>`
      const blob = new Blob([html], { type: 'text/html' })
      const url = URL.createObjectURL(blob)
      const win = window.open(url, '_blank')
      if (!win) alert('Autorise les popups pour exporter le PDF')
    ...
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px', flexWrap: 'wrap', gap: '10px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => setShowDuplicate(!showDuplicate)} style={btn('#EEF2FF', '#0D1B4E', '#4A6FD4')}>📋 Dupliquer</button>
...
const ci = { width: '100%', padding: '10px 12px', border: '1.5px solid #C5D0F0', borderRadius: '8px', fontSize: '15px', fontFamily: "'DM Sans',sans-serif", background: 'white', outline: 'none', color: '#0D1B4E' }
