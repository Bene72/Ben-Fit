...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  async function syncImages(forceAll = false) {
    ...
    try {
      ...
      // 3. Matching intelligent (Auto-match)
      for (const ex of toSync) {
        // Utiliser la fonction findBestExerciseImage qui gère les alias/fautes
        const bestMatch = findBestExerciseImage(ex.name, normalizedFiles)
        if (bestMatch) {
          await supabase.from('exercises').update({ image_url: bestMatch }).eq('id', ex.id)
        }
      }

      await reloadWorkouts()
      alert('✅ Images synchronisées intelligemment !')
    } catch (e) {
  ...
  const duplicateProgram = async (targetClientId) => {
    ...
    try {
      ...
      setDuplicateTarget('')
      alert(`✅ Programme dupliqué avec succès ! (${freshWorkouts.length} séances · ${totalExInserted} exercices)`)
    } catch (e) {
  ...
  const updateExercise = async (workoutId, exId, field, value) => {
    ...
    if (field === 'name') {
      try {
        // Auto-match intelligent lors du renommage
        const bestImage = findBestExerciseImage(value, EXERCISE_IMAGE_FILES)
        if (bestImage) payload.image_url = bestImage
      } catch {}
    }
  ...
  //  FONCTION POUR DÉPLACER UN GROUPE/BLOC ENTIER (Workout Block, Superset, etc.)
  const moveBlockGroup = async (workoutId, groupId, direction) => {
    const w = workouts.find(w => w.id === workoutId)
    if (!w) return
    
    // 1. Extraire tous les exercices et les regrouper par blocs/singles
    const allExercises = [...w.exercises]
    const blocksAndSingles = []
    const processedIds = new Set()

    allExercises.forEach(ex => {
      if (processedIds.has(ex.id)) return
      if (ex.group_id) {
        const group = allExercises.filter(e => e.group_id === ex.group_id)
        group.forEach(e => processedIds.add(e.id))
        blocksAndSingles.push({ type: 'block', items: group, id: ex.group_id })
      } else {
        processedIds.add(ex.id)
        blocksAndSingles.push({ type: 'single', items: [ex], id: ex.id })
      }
    })

    // 2. Trouver le bloc à déplacer
    const currentBlockIndex = blocksAndSingles.findIndex(b => b.id === groupId)
    if (currentBlockIndex === -1) return

    const targetIndex = direction === 'up' ? currentBlockIndex - 1 : currentBlockIndex + 1
    if (targetIndex < 0 || targetIndex >= blocksAndSingles.length) return

    // 3. Swap (Échanger les positions)
    const temp = blocksAndSingles[currentBlockIndex]
    blocksAndSingles[currentBlockIndex] = blocksAndSingles[targetIndex]
    blocksAndSingles[targetIndex] = temp

    // 4. Reconstituer la liste plate et mettre à jour les order_index
    const newExercises = []
    blocksAndSingles.forEach(b => newExercises.push(...b.items))
    newExercises.forEach((ex, idx) => ex.order_index = idx)

    // 5. Mettre à jour l'état local
    setWorkouts(prev => prev.map(w => w.id === workoutId ? { ...w, exercises: newExercises } : w))

    // 6. Sauvegarder dans la DB (uniquement les order_index changés)
    const updates = newExercises.map(ex => 
            ...
            {isOpen && (
              <div style={{ padding: '16px 20px' }}>
                ...
                {workout.exercises?.length > 0 && (
                  <div style={{ display: 'grid', gridTemplateColumns: isEdit ? '24px 1fr 60px 70px 80px 90px 1fr 28px' : '1fr 60px 70px 80px 90px 1fr', gap: '6px', padding: '6px 10px', marginBottom: '4px' }}>
                          ...
                          <div style={{ background: tc, padding: '10px 14px', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            ...
                            {isEdit && (
                              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                {/* 🚀 BOUTONS DÉPLACEMENT BLOC */}
                                <button onClick={() => moveBlockGroup(workout.id, ex.group_id, 'up')} title="Monter" style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', borderRadius: '4px', padding: '4px 8px', fontSize: '12px', cursor: 'pointer' }}>▲</button>
                                <button onClick={() => moveBlockGroup(workout.id, ex.group_id, 'down')} title="Descendre" style={{ background: 'rgba(255,255,255,0.2)', border: 'none', color: 'white', borderRadius: '4px', padding: '4px 8px', fontSize: '12px', cursor: 'pointer' }}>▼</button>
                                <button onClick={() => {
                    ...
                    if (ex.group_id && ex.group_type !== 'Normal') {
                      ...
                      return (
                        <div key={ex.group_id} style={{ border: `2px solid ${groupColors[ex.group_type] || '#C5D0F0'}`, borderRadius: '10px', marginBottom: '10px', overflow: 'hidden' }}>
                          <div style={{ background: groupColors[ex.group_type] || '#C5D0F0', color: 'white', padding: '4px 12px', fontSize: '10px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                            <span>⚡ {ex.group_type}</span>
                            {isEdit && (
                              <div style={{ display: 'flex', gap: '4px', alignItems: 'center' }}>
                                {/* 🚀 BOUTONS DÉPLACEMENT GROUPE */}
                                <button onClick={() => moveBlockGroup(workout.id, ex.group_id, 'up')} title="Monter" style={{ background: 'rgba(255,255,255,0.25)', border: 'none', color: 'white', borderRadius: '4px', padding: '2px 6px', fontSize: '10px', cursor: 'pointer' }}>▲</button>
                                <button onClick={() => moveBlockGroup(workout.id, ex.group_id, 'down')} title="Descendre" style={{ background: 'rgba(255,255,255,0.25)', border: 'none', color: 'white', borderRadius: '4px', padding: '2px 6px', fontSize: '10px', cursor: 'pointer' }}>▼</button>
                                <button onClick={() => addExercise(workout.id, ex.group_type, ex.group_id)} style={{ background: 'rgba(255,255,255,0.25)', border: 'none', color: 'white', borderRadius: '4px', padding: '1px 6px', fontSize: '10px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>+ Exercice</button>
                              </div>
                            )}
                          </div>
...
function ExRow({ ex, wId, edit, onUpdate, onDelete, onMove, isFirst, isLast }) {
  ...
  return (
    <>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: edit ? '24px 1fr 60px 70px 80px 90px 1fr 28px' : '1fr 60px 70px 80px 90px 1fr', gap: '6px', alignItems: 'center', padding: '8px 12px', borderBottom: '1px solid rgba(0,0,0,0.04)' }}>
        {edit && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: '2px' }}>
            <button onClick={() => !isFirst && onMove(wId, ex.id, -1)} disabled={isFirst}
              style={{ width: '22px', height: '18px', border: '1px solid #C5D0F0', borderRadius: '4px', background: isFirst ? '#F5F5F5' : 'white', color: isFirst ? '#CCC' : '#0D1B4E', cursor: isFirst ? 'default' : 'pointer', fontSize: '9px', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 0 }}>▲</button>
