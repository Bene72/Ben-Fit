...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const generateCycle = async () => {
    ...
    try {
      ...
      const prompt = `Tu es un coach expert en musculation et hypertrophie. Tu dois créer un programme d'entraînement pour un(e) client(e).

PROFIL CLIENT:
- Nom: ${profile?.full_name}
- Objectif: ${profile?.objective || 'Non précisé'}
- Programme actuel: ${profile?.current_program || 'Non précisé'}
...
- Assiduité diète: ${lastBilan.assiduite_diete_score}/10 ${lastBilan.assiduite_diete_note ? `(${lastBilan.assiduite_diete_note})` : ''}
- Assiduité training: ${lastBilan.assiduite_training_score}/10 ${lastBilan.assiduite_training_note ? `(${lastBilan.assiduite_training_note})` : ''}
- NEAT: ${lastBilan.neat_score}/10 ${lastBilan.neat_note ? `(${lastBilan.neat_note})` : ''}
- Problèmes diète: ${lastBilan.problemes_diete_note || 'Aucun'}
- Problèmes training: ${lastBilan.problemes_training_note || 'Aucun'}
- Autre: ${lastBilan.autre_note || 'Aucun'}
` : 'Aucun bilan disponible'}

PROGRAMME PRÉCÉDENT (${archived?.[0]?.cycle_name || 'dernier cycle'}):
${archived?.length > 0 ? archived.map(w => 
  `- ${w.name} (${DAYS_FR[(w.day_of_week||1)-1]}): ${(w.exercises||[]).map(e => `${e.name} ${e.sets}x${e.reps}${e.target_weight ? ` @${e.target_weight}` : ''}`).join(', ')}`
).join('
') : 'Aucun cycle précédent'}

PROGRAMME ACTUEL (à remplacer ou améliorer):
${currentWorkouts.length > 0 ? currentWorkouts.map(w =>
  `- ${w.name} (${DAYS_FR[(w.day_of_week||1)-1]}): ${(w.exercises||[]).map(e => `${e.name} ${e.sets}x${e.reps}`).join(', ')}`
).join('
') : 'Aucun programme actuel'}

Génère un nouveau cycle d'entraînement adapté. Réponds UNIQUEMENT en JSON valide avec cette structure exacte:
{
  "cycle_name": "Cycle X — Mois Année",
  "reasoning": "Explication courte de tes choix (2-3 phrases)",
  "workouts": [
    {
      "name": "Nom séance",
      "type": "Push|Pull|Legs|Full Body|Upper|Lower",
      "day_of_week": 1,
      "duration_min": 75,
      "exercises": [
        {
