    ...
    <div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
        {workouts.map(workout => {
          ...
          return (
            <div key={workout.id} style={{ background: '#F0F4FF', border: '1px solid #C5D0F0', borderRadius: '16px', overflow: 'hidden' }}>
              <div style={{ padding: '16px 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px', borderBottom: isOpen ? '1px solid #D9E2F4' : 'none' }}>
                <div onClick={() => { setOpenWorkout(isOpen ? null : workout.id); if (isOpen) setOpenExercise(null) }} style={{ display: 'flex', alignItems: 'center', gap: '12px', cursor: 'pointer', flex: 1 }}>
                  <span style={{ fontSize: '11px', fontWeight: '700', padding: '5px 10px', borderRadius: '999px', background: '#D4E0CC', color: '#0D1B4E' }}>{workout.type}</span>
                  <div>
                    <div style={{ fontWeight: '700', fontSize: '16px', color: '#0D1B4E' }}>{workout.name}</div>
                    <div style={{ fontSize: '13px', color: '#6B7A99' }}>{workout.exercises?.length || 0} exercices · {workout.duration_min} min</div>
                  </div>
                </div>

                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexShrink: 0 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, minmax(70px, 1fr))', gap: '8px' }}>
                    <div style={{ background: 'white', border: '1px solid #DCE5FB', borderRadius: '12px', padding: '8px 10px', minWidth: '70px' }}>
                      <div style={{ fontSize: '10px', color: '#6B7A99', textTransform: 'uppercase', letterSpacing: '1px' }}>Exos</div>
                      <div style={{ fontWeight: '800', fontSize: '18px', color: '#0D1B4E' }}>{workout.exercises?.length || 0}</div>
                    </div>
                    <div style={{ background: 'white', border: '1px solid #DCE5FB', borderRadius: '12px', padding: '8px 10px', minWidth: '70px' }}>
                      <div style={{ fontSize: '10px', color: '#6B7A99', textTransform: 'uppercase', letterSpacing: '1px' }}>Logs</div>
                      <div style={{ fontWeight: '800', fontSize: '18px', color: '#0D1B4E' }}>{logCount}</div>
                    </div>
                    <div style={{ background: 'white', border: '1px solid #DCE5FB', borderRadius: '12px', padding: '8px 10px', minWidth: '70px' }}>
                      <div style={{ fontSize: '10px', color: '#6B7A99', textTransform: 'uppercase', letterSpacing: '1px' }}>État</div>
                      <div style={{ fontWeight: '800', fontSize: '15px', color: '#0D1B4E' }}>{isOpen ? 'Ouvert' : 'Fermé'}</div>
                    </div>
                  </div>

                  <select value={workout.day_of_week || 1} onChange={e => updateWorkoutDay(workout.id, e.target.value)} style={{ ...inp, width: '120px' }}>
              ...
              {isOpen ? (
                <div style={{ padding: '18px 20px' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: '12px', marginBottom: '14px', flexWrap: 'wrap' }}>
                    <div>
                      <div style={{ fontWeight: '700', fontSize: '18px', color: '#0D1B4E' }}>{workout.name}</div>
                      <div style={{ fontSize: '13px', color: '#6B7A99' }}>
                        Même logique que la vue client : clique un exercice pour ouvrir son détail.
                      </div>
                    </div>
                    ...
                    <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
                      ...
                      {isEdit ? (
                        <button onClick={() => deleteWorkout(workout.id)} style={{ ...btn('rgba(196,92,58,0.1)', '#C45C3A'), marginLeft: 'auto' }}>
                          🗑 Supprimer
                        </button>
                      ) : null}
                    </div>
                  ...
                  {isEdit ? (
                    <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 140px 140px', gap: '10px', marginBottom: '16px' }}>
                      <div>
                        <label style={lbl}>Nom séance</label>
                        <input value={workout.name} onChange={e => updateWorkoutField(workout.id, 'name', e.target.value)} style={inp} />
                      </div>
                      <div>
                        <label style={lbl}>Type</label>
                        <select value={workout.type || 'Push'} onChange={e => updateWorkoutField(workout.id, 'type', e.target.value)} style={inp}>
                          {['Push','Pull','Legs','Full Body','Upper','Lower','Cardio','Autre'].map(t => <option key={t}>{t}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={lbl}>Jour</label>
                        <select value={workout.day_of_week || 1} onChange={e => updateWorkoutDay(workout.id, e.target.value)} style={inp}>
                          {DAYS_FR.map((d, i) => <option key={d} value={i + 1}>{d}</option>)}
                        </select>
                      </div>
                      <div>
                        <label style={lbl}>Durée</label>
                        <input type="number" value={workout.duration_min || 60} onChange={e => updateWorkoutField(workout.id, 'duration_min', +e.target.value)} style={inp} />
                      </div>
                    </div>
                  ) : null}
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                    {groupedExercises(workout).map((item, idx) => {
                      if (item.kind === 'group') {
                        return (
                          <div key={item.groupId || idx} style={{ border: `2px solid ${groupColors[item.groupType] || '#C5D0F0'}`, borderRadius: '14px', overflow: 'hidden', background: '#FFFFFF' }}>
                            <div style={{ background: groupColors[item.groupType] || '#C5D0F0', color: 'white', padding: '6px 12px', fontSize: '11px', fontWeight: '700', letterSpacing: '1px', textTransform: 'uppercase', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                              <span>{item.groupType}</span>
                              {isEdit ? (
                                <button onClick={() => addExercise(workout.id, item.groupType, item.groupId)} style={{ background: 'rgba(255,255,255,0.18)', border: 'none', color: 'white', borderRadius: '6px', padding: '3px 8px', fontSize: '11px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
                                  + Exercice
                                </button>
                              ) : null}
                            </div>
                  ...
                  {workout.exercises?.length === 0 && !isEdit ? (
                    <div style={{ textAlign: 'center', color: '#6B7A99', fontSize: '13px', padding: '18px' }}>
