...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const confirmAddExercise = async (name, imageUrl) => {
    ...
    const payload = {
      ...
      group_id: gid,
      // image_url stockée si la colonne existe, sinon ignorée
    }
    
    // Tente avec image_url, fallback sans si colonne manquante
    let data = null, error = null
    try {
      const res = await supabase.from('exercises').insert({ ...payload, image_url: imageUrl || null }).select().single()
      data = res.data; error = res.error
      if (error?.message?.includes('image_url')) {
        // Colonne absente → insérer sans
        const res2 = await supabase.from('exercises').insert(payload).select().single()
    ...
    if (data) {
      // Ajouter image_url en mémoire locale même si pas en BDD
      const exWithImg = { ...data, image_url: imageUrl || null }
      setWorkouts(prev => prev.map(w => {
        if (w.id === workoutId) {
          return { 
            ...w, 
            exercises: [...(w.exercises || []), exWithImg].sort((a, b) => (a.order_index || 0) - (b.order_index || 0))
          }
        }
        return w
      }))
  ...
  const updateExercise = async (workoutId, exId, field, value) => {
    ...
    // Tente update avec image_url, fallback sans si colonne manquante
    const { error: updateError } = await supabase.from('exercises').update(payload).eq('id', exId)
    if (updateError?.message?.includes('image_url')) {
      const { image_url: _, ...payloadWithout } = payload
      await supabase.from('exercises').update(payloadWithout).eq('id', exId)
    }
  ...
  const saveCurrentCycleName = async () => {
    ...
    // Stocker le nom dans tous les workouts actifs du client
    await supabase.from('workouts')
      .update({ cycle_name: currentCycleName.trim() })
      .eq('client_id', clientId)
      .eq('is_archived', false)
    setSavingCycleName(false)
    alert('✓ Nom du cycle enregistré')
  }

  // ── Archiver le cycle ────────────────────────────────────────
  const archiveCycle = async () => {
    if (!cycleName.trim()) { alert("Donne un nom à ce cycle avant de l'archiver"); return }
    if (!confirm(`Archiver le cycle "${cycleName}" ? Les séances seront archivées et le programme sera vidé.`)) return
    setArchiving(true)
    try {
      await supabase.from('workouts')
      ...
      for (const ex of toSync) {
        ...
        if (match) {
          try {
            ...
            if (sErr?.message?.includes('image_url')) {
              console.warn('Colonne image_url absente - ajoute-la avec: ALTER TABLE exercises ADD COLUMN image_url text;')
            }
          } catch {}
        }
      }
...
const btn = (bg, color, border, fs) => ({ padding: `7px 14px`, background: bg, color, border: border ? `1.5px solid ${border}` : 'none', borderRadius: '8px', fontSize: fs||'13px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" })      {/* Header programme */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '12px' }}>
        ...
        <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap' }}>
          ...
          {workouts.length > 0 && <button onClick={exportPDF} disabled={exporting} style={btn('#C45C3A', 'white')}>{exporting ? '⏳...' : '⬇️ Export PDF'}</button>}
          <button onClick={() => setShowAdd(true)} style={btn('#0D1B4E', 'white')}>+ Nouvelle séance</button>
        </div>
      </div>
      ...
      {workouts.length > 0 && (
        <div style={{ background: '#FFF8E1', border: '1px solid #FFD54F', borderRadius: '10px', padding: '10px 14px', marginBottom: '10px', display: 'flex', gap: '10px', alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          <span style={{ fontSize: '10px', color: '#9E8000' }}>⚠️ Sera archive et le programme vide.</span>
        </div>
      )}

      {/* Historique des cycles archives */}
      {showHistory && (
        <div style={{ background: '#F0F4FF', border: '2px solid #4A6FD4', borderRadius: '12px', padding: '16px 20px', marginBottom: '14px' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '14px' }}>
            ...
            <button onClick={() => setShowHistory(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#9BA8C0', fontSize: '18px' }}>×</button>
          </div>
          {archivedWorkouts.length === 0 ? (
            <div style={{ color: '#9BA8C0', fontSize: '13px', textAlign: 'center', padding: '12px' }}>Aucun cycle archive pour ce client.</div>
          ) : (
            (() => {
              const cycles = {}
              archivedWorkouts.forEach(w => {
                const key = w.cycle_name || 'Sans nom'
                if (!cycles[key]) cycles[key] = { workouts: [], archived_at: w.archived_at }
                cycles[key].workouts.push(w)
              })
              return Object.entries(cycles).map(([name, { workouts: cw, archived_at }]) => (
                <details key={name} style={{ marginBottom: '8px', background: 'white', borderRadius: '9px', border: '1px solid #C5D0F0', overflow: 'hidden' }}>
                  <summary style={{ padding: '10px 14px', cursor: 'pointer', fontWeight: '700', fontSize: '13px', color: '#0D1B4E', listStyle: 'none', display: 'flex', justifyContent: 'space-between' }}>
                    <span>📌 {name}</span>
                    <span style={{ fontSize: '11px', color: '#9BA8C0', fontWeight: '400' }}>
                      {cw.length} seance{cw.length > 1 ? 's' : ''} · {archived_at ? new Date(archived_at).toLocaleDateString('fr-FR') : '—'}
                    </span>
                  </summary>
                    ...
                    {cw.map(w => (
                      <div key={w.id} style={{ marginBottom: '8px' }}>
                        ...
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px', marginTop: '4px' }}>
                          {(w.exercises || []).slice(0, 8).map(e => (
                            <span key={e.id} style={{ fontSize: '11px', background: '#EEF2FF', color: '#4A6FD4', padding: '2px 8px', borderRadius: '20px' }}>{e.name}</span>
                          ))}
