...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  // Liste des fichiers — initialisée avec le fallback, enrichie dynamiquement via l'API
  const STATIC_FILES = [
  ...
  const [exerciseImageFiles, setExerciseImageFiles] = useState(STATIC_FILES)
  ...
  useEffect(() => {
    fetch('/api/exercise-images')
      ...
      .then(d => {
        ...
        if (files.length > 0) setExerciseImageFiles(files.sort((a, b) => a.localeCompare(b, 'fr')))
      })
      .catch(() => {
        // Garde le fallback statique déjà en place
        setExerciseImageFiles(STATIC_FILES)
      })
  }, [])
...
// ─── EXERCISE PICKER MODAL ──────────────────────────────────
function ExercisePicker({ picker, query, setQuery, mode, setMode, freeVal, setFreeVal, onConfirm, onClose, exerciseFiles, supabaseUrl }) {
