...
import { NutritionCoachPanel } from '../components/nutrition/NutritionShared'
...
function validateClientForm(newClient) {
  ...
  if (!full_name || !email || !password) {
    return { valid: false, message: 'Tous les champs sont obligatoires' }
  }
  if (!/^\S+@\S+\.\S+$/.test(email)) {
    return { valid: false, message: 'Adresse email invalide' }
  }
  if (password.length < 6) {
    return { valid: false, message: 'Le mot de passe doit faire au moins 6 caractères' }
  }
  return { valid: true, payload: { full_name, email, password } }
}
...
export default function CoachPanel() {
  ...
  const loadClients = async (coachId) => {
    ...
    setSelected(prev => {
      if (!prev) return nextClients[0] || null
      const fresh = nextClients.find(client => client.id === prev.id)
      return fresh || nextClients[0] || null
    })
  ...
  const loadPanel = async () => {
    ...
    try {
      ...
      console.error('Coach panel load error:', error)
      setLoadError(error?.message || 'Impossible de charger le panel coach.')
    } finally {
      setLoading(false)
    }
  }
  ...
  const refreshClients = async () => {
    ...
    try {
      ...
      setLoadError(error?.message || 'Impossible de rafraîchir la liste des clients.')
    } finally {
  ...
  const createClient = async () => {
    ...
    try {
      const { data: { session } } = await supabase.auth.getSession()
      if (!session?.user?.id) throw new Error('Session expirée, veuillez vous reconnecter')
      ...
      await callEdgeFunction('create-client', payload)
      await loadClients(session.user.id)
      setShowNewClient(false)
      setCreateSuccess({ name: payload.full_name, email: payload.email, password: payload.password })
      setNewClient({ full_name: '', email: '', password: '' })
    } catch (error) {
      console.error('Create client error:', error)
      setCreateError('Erreur: ' + (error?.message || 'Impossible de créer le client'))
    } finally {
      setCreating(false)
    }
  }
  ...
  const sessionsThisWeek = (client) => {
    ...
    return (client.workout_sessions || []).filter(session => new Date(session.date) >= weekStart).length
  }

  const lastWeight = (client) => {
    const latestMeasure = [...(client.measures || [])].sort((a, b) => new Date(b.date) - new Date(a.date))
    return latestMeasure[0]?.weight || '—'
  }
  ...
  if (loadError) {
    return (
      <>
        ...
        <ErrorScreen message={loadError} onRetry={loadPanel} />
      </>
    )
  }
      ...
      <div style={{ display: 'flex', minHeight: '100vh', background: '#EEF0F5', fontFamily: "'DM Sans',sans-serif" }}>
        ...
        <aside style={{ width: sidebarOpen ? '260px' : '0px', background: '#0D1B4E', position: 'fixed', top: 0, bottom: 0, left: 0, display: 'flex', flexDirection: 'column', zIndex: 100, overflow: 'hidden', transition: 'width 0.25s ease' }}>
          ...
          <div style={{ padding: '12px 12px 4px' }}>
            <div style={{ display: 'flex', gap: '8px', marginBottom: '10px' }}>
              ...
              <button
                ...
                style={{
                  ...
                  border: '1px solid rgba(255,255,255,0.12)',
                  borderRadius: '8px',
                  cursor: refreshingClients ? 'not-allowed' : 'pointer',
                  fontSize: '14px'
                }}
                title="Rafraîchir"
              >
            ...
            <div style={{ fontSize: '10px', letterSpacing: '2px', textTransform: 'uppercase', color: '#6B7A99', padding: '4px 4px 0' }}>
          ...
          <div style={{ flex: 1, padding: '8px 12px 12px', overflowY: 'auto' }}>
            {filteredClients.length === 0 ? (
              <div style={{ color: '#6B7A99', fontSize: '12px', textAlign: 'center', padding: '24px 12px' }}>
                Aucun client trouvé.
              </div>
            ) : (
              filteredClients.map(client => {
                const isSelected = selected?.id === client.id
                const weekSessions = sessionsThisWeek(client)
                return (
                  <button
                    key={client.id}
                    ...
                    style={{
                      ...
                      borderRadius: '8px',
                      cursor: 'pointer',
                      background: isSelected ? '#162862' : 'transparent',
                      border: 'none',
                      width: '100%',
                      textAlign: 'left',
                      fontFamily: "'DM Sans',sans-serif",
                  ...
                  >
                    ...
                    <div style={{ flex: 1, minWidth: 0 }}>
                      <div style={{ fontSize: '13px', color: isSelected ? 'white' : '#C8D1EE', fontWeight: '500', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                        {client.full_name}
                      </div>
                      <div style={{ fontSize: '11px', color: '#8FA1C9' }}>{weekSessions}/5 séances · {lastWeight(client)} kg</div>
                    </div>
                    <div style={{ position: 'relative', flexShrink: 0 }}>
                      <span style={{ width: '7px', height: '7px', borderRadius: '50%', background: weekSessions >= 4 ? '#8FA07A' : weekSessions <= 1 ? '#C45C3A' : '#4A6FD4', display: 'block' }} />
          ...
          <div style={{ padding: '16px', borderTop: '1px solid rgba(255,255,255,0.08)', display: 'flex', flexDirection: 'column', gap: '8px' }}>
            ...
            <button onClick={async () => { await supabase.auth.signOut(); router.push('/') }} style={{ width: '100%', padding: '8px', background: 'transparent', border: '1px solid rgba(255,255,255,0.12)', borderRadius: '8px', color: '#C8D1EE', fontSize: '12px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>Déconnexion</button>
          </div>

          {showNewClient && (
            <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              <div style={{ background: '#0D1B4E', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '28px', width: '380px', fontFamily: "'DM Sans',sans-serif" }}>
                ...
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#8FA1C9', marginBottom: '5px' }}>Nom complet</label>
                  <input value={newClient.full_name} onChange={e => setNewClient(prev => ({ ...prev, full_name: e.target.value }))} placeholder="Jean Dupont" style={{ width: '100%', padding: '9px 12px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: 'rgba(255,255,255,0.07)', color: 'white', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: '12px' }}>
                  <label style={{ display: 'block', fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#8FA1C9', marginBottom: '5px' }}>Email</label>
                  <input type="email" value={newClient.email} onChange={e => setNewClient(prev => ({ ...prev, email: e.target.value }))} placeholder="jean@email.com" style={{ width: '100%', padding: '9px 12px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: 'rgba(255,255,255,0.07)', color: 'white', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ marginBottom: '8px' }}>
                  <label style={{ display: 'block', fontSize: '11px', letterSpacing: '1.5px', textTransform: 'uppercase', color: '#8FA1C9', marginBottom: '5px' }}>Mot de passe temporaire</label>
                  <input value={newClient.password} onChange={e => setNewClient(prev => ({ ...prev, password: e.target.value }))} placeholder="MotDePasse123!" style={{ width: '100%', padding: '9px 12px', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', fontSize: '14px', fontFamily: "'DM Sans',sans-serif", background: 'rgba(255,255,255,0.07)', color: 'white', outline: 'none', boxSizing: 'border-box' }} />
                </div>
                <div style={{ fontSize: '11px', color: 'rgba(255,255,255,0.45)', marginBottom: '16px' }}>
                  Minimum 6 caractères. L'email sera normalisé en minuscules.
                </div>
                ...
                <div style={{ display: 'flex', gap: '8px' }}>
                  ...
                  <button onClick={closeNewClientModal} style={{ padding: '10px 16px', background: 'transparent', color: '#C8D1EE', border: '1px solid rgba(255,255,255,0.15)', borderRadius: '8px', fontSize: '13px', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>Annuler</button>
                </div>
              </div>
            </div>
          )}
        ...
        {createSuccess && (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 200, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <div style={{ background: '#0D1B4E', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '16px', padding: '28px', width: '400px', fontFamily: "'DM Sans',sans-serif" }}>
              ...
              <div style={{ background: 'rgba(255,255,255,0.07)', borderRadius: '10px', padding: '16px', marginBottom: '20px' }}>
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '8px', gap: '12px' }}>
                  ...
                  <span style={{ fontSize: '13px', color: 'white', fontWeight: '600', textAlign: 'right', wordBreak: 'break-all' }}>{createSuccess.email}</span>
                </div>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: '12px' }}>
                  <span style={{ fontSize: '12px', color: 'rgba(255,255,255,0.5)' }}>Mot de passe</span>
                  <span style={{ fontSize: '13px', color: 'white', fontWeight: '600', textAlign: 'right', wordBreak: 'break-word' }}>{createSuccess.password}</span>
                </div>
              </div>
        ...
        <main style={{ marginLeft: sidebarOpen ? '260px' : '0px', flex: 1, transition: 'margin-left 0.25s ease' }}>
          {!selected ? (
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh', flexDirection: 'column', gap: '12px', color: '#6B7A99' }}>
              ...
              <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '24px', letterSpacing: '3px', color: '#0D1B4E' }}>
                {clients.length === 0 ? 'AUCUN CLIENT POUR LE MOMENT' : 'SÉLECTIONNE UN CLIENT'}
              </div>
              {clients.length === 0 && (
                <button onClick={() => setShowNewClient(true)} style={{ padding: '10px 18px', background: '#0D1B4E', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
            ...
            <>
              <div style={{ padding: '16px 32px', borderBottom: '1px solid #C5D0F0', background: '#EEF2FF', position: 'sticky', top: 0, zIndex: 50, display: 'flex', alignItems: 'center', gap: '16px' }}>
                ...
                <div style={{ marginLeft: 'auto', display: 'flex', gap: '6px', flexWrap: 'wrap' }}>
                  {[['overview',"👁 Vue d'ensemble"],['programme','🏋️ Programme'],['nutrition','🥗 Nutrition'],['bilan','📋 Bilan'],['messages','💬 Messages'],['gestion','⚙️ Gestion']].map(([tabId, label]) => (
                    <button key={tabId} onClick={() => setTab(tabId)} style={{ padding: '7px 14px', borderRadius: '8px', fontSize: '13px', fontWeight: '500', cursor: 'pointer', border: 'none', background: tab === tabId ? (tabId === 'gestion' ? '#C45C3A' : '#0D1B4E') : 'transparent', color: tab === tabId ? 'white' : '#6B7A99', fontFamily: "'DM Sans',sans-serif" }}>{label}</button>
                  ))}
                </div>
              </div>

              <div style={{ padding: '28px 32px' }}>
                {tab === 'overview' && <OverviewTab client={selected} sessionsThisWeek={sessionsThisWeek} lastWeight={lastWeight} coachId={user.id} onUpdate={(updated) => { setSelected(updated); setClients(prev => prev.map(client => client.id === updated.id ? updated : client)) }} />}
                ...
                {tab === 'gestion' && <GestionTab client={selected} session={null} onDelete={handleDeleteClient} />}
              </div>
            </>
          )}
        </main>
...
function ErrorScreen({ message, onRetry }) {
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#EEF2FF', padding: '24px', fontFamily: "'DM Sans',sans-serif" }}>
      <div style={{ maxWidth: '460px', width: '100%', background: 'white', border: '1px solid #D9E1F7', borderRadius: '18px', padding: '28px', textAlign: 'center', boxShadow: '0 10px 30px rgba(13,27,78,0.08)' }}>
        <div style={{ fontSize: '38px', marginBottom: '12px' }}>⚠️</div>
        <div style={{ fontFamily: "'Bebas Neue',sans-serif", fontSize: '24px', letterSpacing: '2px', color: '#0D1B4E', marginBottom: '10px' }}>CHARGEMENT IMPOSSIBLE</div>
        <div style={{ fontSize: '14px', color: '#6B7A99', lineHeight: '1.6', marginBottom: '18px' }}>{message}</div>
        <button onClick={onRetry} style={{ padding: '10px 18px', background: '#0D1B4E', color: 'white', border: 'none', borderRadius: '10px', fontWeight: '600', cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
          Réessayer
        </button>
      </div>
    </div>
...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  const generateCycle = async () => {
    ...
    try {
      ...
      // 2. Dernier bilan
      const { data: bilans } = await supabase.from('bilans').select('*')
...
function NutritionTab({ clientId, clientName }) {
  return <NutritionCoachPanel clientId={clientId} clientName={clientName} />
}
