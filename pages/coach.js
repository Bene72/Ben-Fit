...
function ProgrammeTab({ clientId, clientName, coachId }) {
  ...
  // Liste dynamique des fichiers du bucket — chargée depuis l'API route
  const [exerciseImageFiles, setExerciseImageFiles] = useState([])

  useEffect(() => {
    fetch('/api/exercise-images')
      .then(r => r.json())
      .then(d => setExerciseImageFiles((d.files || []).sort((a, b) => a.localeCompare(b, 'fr'))))
      .catch(() => {
  ...
  // Alias pour la compat avec le reste du code
