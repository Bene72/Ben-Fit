...
const NUTRI_FIELDS = [
  { key: 'calories', label: 'Calories', unit: 'kcal', icon: '🔥', color: 'var(--danger)' },
  { key: 'protein', label: 'Protéines', unit: 'g', icon: '🥩', color: '#2C8A6E' },
  { key: 'carbs', label: 'Glucides', unit: 'g', icon: '🌾', color: 'var(--gold)' },
...
function NutritionMiniChart({ logs, field }) {
  const data = [...logs]
    .filter((l) => l[field] != null && l[field] !== 0)
    .sort((a, b) => new Date(a.date) - new Date(b.date))
    .slice(-30)
  const fieldMeta = NUTRI_FIELDS.find((f) => f.key === field)
  const color = fieldMeta?.color || 'var(--accent)'
  if (data.length < 2)
    return (
      <div
        style={{
      ...
      >
        ...
        <div style={{ fontSize: 12, color: '#6B7A99' }}>
          Ajoute au moins 2 jours de suivi pour voir la courbe
        </div>
      </div>
    )
  ...
  const pts = data.map((l, i) => [
    ...
    PY + ((max - +l[field]) / range) * (H - PY * 2 - 14),
  ])
  ...
  return (
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 140, overflow: 'visible' }}>
        ...
        <text x={W - PX + 3} y={H - PY - 12} fontSize="9" fill="#6B7A99">
          {min}
        </text>
      </svg>
      <div
      ...
      >
        ...
        <div style={{ fontSize: 12, fontWeight: 800, color: isPos ? 'var(--danger)' : '#2C8A6E' }}>
          {isPos ? '+' : ''}
...
function NutritionHistory({ logs, isMobile, onOpenDay }) {
  ...
  return (
    <SurfaceCard padded>
      <SectionHead
        title="Historique"
        caption="Toutes les valeurs de diète enregistrées, avec la date, et leur évolution en courbe."
      />
        ...
        {[
          ...
          <button
            ...
            style={{
              ...
              cursor: 'pointer',
              color: subTab === t.id ? 'var(--accent)' : '#6B7A99',
        ...
        sorted.length === 0 ? (
          ...
          >
            {sorted.map((l, i) => (
              <div
                key={l.id || l.date}
                onClick={() => onOpenDay && onOpenDay(l.date)}
                style={{
              ...
              >
                ...
                >
                  <span
                    style={{ fontSize: 12, color: '#6B7A99', fontFamily: "'DM Mono',monospace" }}
                  >
                  ...
                  {NUTRI_FIELDS.filter((f) => l[f.key] != null).map((f) => (
                    <div key={f.key} style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                      ...
                      <span style={{ fontWeight: 900, fontSize: 15, color: f.color }}>
                        {l[f.key]}
                        <span style={{ fontSize: 10, fontWeight: 400, color: '#6B7A99' }}>
                          {' '}
                          {f.unit}
                        </span>
                      </span>
        ...
        <div>
            ...
            {NUTRI_FIELDS.filter((f) => logs.some((l) => l[f.key] != null)).map((f) => (
              <button
                ...
                style={{
                  ...
                  fontSize: 12,
                  fontWeight: 700,
                  background: field === f.key ? f.color : '#EAF0F8',
                  color: field === f.key ? 'white' : '#6B7A99',
                }}
              >
                {f.icon} {f.label}
              </button>
          ...
          <div style={{ background: '#F8FBFF', borderRadius: 12, padding: '16px 14px' }}>
            <div style={{ fontWeight: 800, fontSize: 14, color: 'var(--navy)', marginBottom: 12 }}>
              {NUTRI_FIELDS.find((f) => f.key === field)?.icon}{' '}
              {NUTRI_FIELDS.find((f) => f.key === field)?.label}{' '}
              <span style={{ fontSize: 11, color: '#6B7A99', fontWeight: 400 }}>
                ({NUTRI_FIELDS.find((f) => f.key === field)?.unit})
              </span>
            </div>
...
export default function NutritionPage() {
  ...
  async function ensureLog() {
    ...
    return await saveLog(
      selectedDate,
      { calories: 0, protein: 0, carbs: 0, fat: 0, note: null },
      true
    )
  }
  ...
  return (
    <AppShell
      title="Nutrition"
      subtitle="Tu peux renseigner tes macros ou tes aliments dans l'ordre que tu veux."
      actions={<SegmentTabs items={NUTRITION_TABS} value={activeTab} onChange={setActiveTab} />}
      userName={userName}
      cycleName={cycleName}
      coachName="Ben"
      ...
      <div
        style={{
          ...
          gridTemplateColumns: isMobile
            ? '1fr'
            : 'minmax(250px,0.85fr) minmax(0,1.6fr) minmax(220px,0.7fr)',
          gap: 14,
        }}
      >
          ...
          <SurfaceCard padded sticky={!isMobile}>
            ...
            {plan ? (
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                >
                  <div style={{ fontWeight: 900, color: 'var(--navy)', marginBottom: 8 }}>
                    Notes
                  </div>
                  <div style={{ color: '#6B7A99', lineHeight: 1.7 }}>
                    {plan.notes || plan.coach_note || 'Aucune note nutritionnelle.'}
                  </div>
                </div>
              </div>
            ) : (
            ...
            <>
              ...
              <SurfaceCard padded>
                ...
                >
                  <KpiCard
                    label="Calories"
                    value={combinedValues.calories}
                    caption={`${clampPercent(combinedValues.calories, plan?.target_calories)}% de l'objectif`}
                  />
                  <KpiCard
                    label="Protéines"
                    value={combinedValues.protein}
                    caption={`${clampPercent(combinedValues.protein, plan?.target_protein)}% de l'objectif`}
                  />
                  <KpiCard
                    label="Glucides"
                    value={combinedValues.carbs}
                    caption={`${clampPercent(combinedValues.carbs, plan?.target_carbs)}% de l'objectif`}
                  />
                </div>
                <div
                  style={{
                ...
                >
                  <div style={{ color: '#6B7A99', fontSize: 13, lineHeight: 1.6 }}>
                    Les valeurs du détail aliments s'ajoutent automatiquement.
                  </div>
                  <button
                    type="button"
                    onClick={() => saveLog()}
              ...
              <SurfaceCard padded>
                <SectionHead
                  ...
                  caption="Comparaison par rapport au plan du jour."
                />
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  <ProgressBar
                    label="Calories"
                    ...
                    percent={clampPercent(combinedValues.calories, plan?.target_calories)}
                  />
                  <ProgressBar
                    label="Protéines"
                    value={combinedValues.protein}
                    target={plan?.target_protein}
                    percent={clampPercent(combinedValues.protein, plan?.target_protein)}
                  />
                  <ProgressBar
                    label="Glucides"
                    value={combinedValues.carbs}
                    target={plan?.target_carbs}
                    percent={clampPercent(combinedValues.carbs, plan?.target_carbs)}
                  />
                  <ProgressBar
                    label="Lipides"
                    value={combinedValues.fat}
                    target={plan?.target_fat}
                    percent={clampPercent(combinedValues.fat, plan?.target_fat)}
                  />
                </div>
              </SurfaceCard>
            </>
            ...
            <SurfaceCard padded sticky>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                <ListMetric
                  label="Calories"
                  value={combinedValues.calories}
                  success={clampPercent(combinedValues.calories, plan?.target_calories) >= 100}
                />
                <ListMetric
                  label="Protéines"
                  value={`${combinedValues.protein} g`}
                  success={clampPercent(combinedValues.protein, plan?.target_protein) >= 100}
                />
                <ListMetric
                  label="Glucides"
                  value={`${combinedValues.carbs} g`}
                  success={clampPercent(combinedValues.carbs, plan?.target_carbs) >= 100}
                />
                <ListMetric
                  label="Lipides"
                  value={`${combinedValues.fat} g`}
                  success={clampPercent(combinedValues.fat, plan?.target_fat) >= 100}
                />
              </div>
            </SurfaceCard>
          </div>
  ...
  return (
    <div>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
        <div style={{ fontWeight: 800, color: 'var(--navy)' }}>{label}</div>
...
function ListMetric({ label, value, success = false }) {
  return (
    ...
    >
      <div style={{ color: 'var(--navy)', fontWeight: 800 }}>{label}</div>
...
function KpiCard({ label, value, caption }) {
  return (
    ...
    >
      ...
      <div style={{ fontWeight: 900, fontSize: 22, color: 'var(--navy)', lineHeight: 1 }}>
        {value}
      </div>
