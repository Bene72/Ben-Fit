...
export default function Nutrition({ clientId }) {
  ...
  return (
    <Layout title={isMobile ? "" : "Nutrition"} user={user}>
      ...
      {isMobile ? (
        <div style={{ marginTop: '-10px', marginBottom: 14 }}>
          ...
          <div style={{ fontWeight: 900, fontSize: 22, color: '#0D1B4E', marginBottom: 4 }}>Nutrition</div>
          <div style={{ color: '#6B7A99', fontSize: 13, lineHeight: 1.45 }}>Un tableau de bord plus clair pour suivre tes apports et comparer facilement avec le plan.</div>
        </div>
      ) : null}
