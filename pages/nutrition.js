            ...
            {macros.map(m => (
              <div key={m.key}>
                ...
                <input type="number" value={form[m.key]} onChange={e => setForm(p=>({...p,[m.key]:e.target.value}))} placeholder={m.target||'0'}
                  style={{ width:'100%', padding:'9px 10px', border:`2px solid ${m.color}33`, borderRadius:'8px', fontSize:'14px', fontFamily:"'DM Sans',sans-serif", outline:'none', background:m.bg }} />
              ...
              {macros.map(m => {
                ...
                return (
                  <div key={m.key}>
                    ...
                    {m.target && <div style={{ fontSize:'10px', color:'#BBB', marginTop:'3px', textAlign:'right' }}>{Math.round(pct)}% / {m.target}{m.unit}</div>}
                  </div>
                )
              })}
            </div>
