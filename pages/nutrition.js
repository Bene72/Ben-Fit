...
import { fetchNutritionData, upsertNutritionLogRecord } from '../lib/services/nutrition'
...
export default function Nutrition({ clientId }) {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const { plan, logs } = await fetchNutritionData(userId)
        setPlan(plan)
        setLogs(logs)
      } catch (e) {
        setError(e.message || 'Erreur de chargement nutrition')
      } finally {
        setLoading(false)
      }
    }
  ...
  const upsertLog = async (date, fields) => {
    try {
      ...
      const data = await upsertNutritionLogRecord({
      ...
      setError(e.message || 'Erreur de sauvegarde nutrition')
      return null
    }
  }
  ...
  if (clientId) return (
    <div style={{ fontFamily:"'DM Sans',sans-serif" }}>
      {error && <div style={{ background:'#FFF4F4', border:'1px solid #F1C0C0', color:'#9A2A2A', borderRadius:'12px', padding:'12px 16px', marginBottom:'16px', fontSize:'13px' }}>{error}</div>}
  ...
  return (
    <Layout title="Nutrition" user={user}>
      {error && <div style={{ background:'#FFF4F4', border:'1px solid #F1C0C0', color:'#9A2A2A', borderRadius:'12px', padding:'12px 16px', marginBottom:'16px', fontSize:'13px', fontFamily:"'DM Sans',sans-serif" }}>{error}</div>}
...
function FoodDetailBlock({ log: initialLog, date, onSave, onItemsChange }) {
  ...
  // Crée un log vide si pas encore de log pour ce jour
    ...
    <div style={{ background:'white', borderRadius:'14px', border:'1px solid #EAEAEA', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.06)' }}>
      ...
      {items.length === 0 ? (
        <div style={{ padding:'24px', textAlign:'center', color:'#CCC', fontSize:'13px' }}>
          Aucun aliment ajouté — clique sur "🔍 Rechercher" ou "✏️ Libre"
        </div>
      ) : (
