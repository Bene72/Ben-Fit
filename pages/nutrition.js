...
// ============================================
const WEEK_MACROS = [
  { key: 'calories', label: 'Kcal',     unit: 'kcal', target: 'target_calories', color: '#0D1B4E' },
  { key: 'protein',  label: 'Prot.',    unit: 'g',    target: 'target_protein',  color: '#C45C3A' },
  { key: 'carbs',    label: 'Gluc.',    unit: 'g',    target: 'target_carbs',    color: '#2A50B0' },
  { key: 'fat',      label: 'Lip.',     unit: 'g',    target: 'target_fat',      color: '#3A7BD5' },
]
...
function NutritionWeekTable({ logs, plan, isMobile, onOpenDay }) {
  ...
  const sortedWeeks = useMemo(() =>
    Object.keys(weeks).sort((a, b) => b.localeCompare(a)), [weeks])

  const getWeekLabel = (wk) => {
    const s = new Date(wk + 'T12:00:00')
    const e = new Date(wk + 'T12:00:00')
  ...
  const toggle = (date, isFuture) => {
    if (!isFuture) setOpenDay(o => o === date ? null : date)
  }
    ...
    return (
      <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
        {sortedWeeks.map(weekStart => {
          ...
          const days = getDays(weekStart, weekLogs)
          const filledDays = weekLogs.filter(l => l.calories > 0).length

          return (
            <div key={weekStart} style={{ background: 'white', borderRadius: 14, border: `1.5px solid ${isCurrent ? '#C0CAEF' : '#EAEAEA'}`, overflow: 'hidden', boxShadow: '0 2px 8px rgba(13,27,78,0.05)' }}>
              {/* Header semaine */}
              <div style={{ padding: '10px 14px', background: isCurrent ? '#EEF2FF' : '#F5F7FF', borderBottom: '1px solid #EAEAEA', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ fontWeight: 700, fontSize: 13, color: '#0D1B4E' }}>📅 {getWeekLabel(weekStart)}</div>
                <div style={{ fontSize: 11, color: '#999', background: '#F0F0F5', padding: '2px 8px', borderRadius: 20 }}>{filledDays}/7j</div>
              </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column' }}>
                {days.map(({ date, log, isToday, isFuture }) => {
                  ...
                  const di = new Date(date + 'T12:00:00').getDay()
                  const dayName = DAYS_FR[di === 0 ? 6 : di - 1]
                  const hasData = log && log.calories > 0

                  return (
                    <div key={date} style={{ borderBottom: '1px solid #F5F5F5' }}>
                      <div onClick={() => toggle(date, isFuture)} style={{
                        display: 'flex', alignItems: 'center', gap: 10,
                        padding: '10px 14px',
                        background: isToday ? '#F0F4FF' : isOpen ? '#F5F8FF' : 'transparent',
                        cursor: isFuture ? 'default' : 'pointer',
                      }}>
                        {/* Jour */}
                        <div style={{ minWidth: 70 }}>
                          <div style={{ fontSize: 12, fontWeight: isToday ? 800 : 500, color: isFuture ? '#CCC' : '#0D1B4E' }}>
                            {isToday ? '📍 ' : ''}{dayName}
                          </div>
                          <div style={{ fontSize: 10, color: '#9BA8C0' }}>
                            {new Date(date + 'T12:00:00').toLocaleDateString('fr-FR', { day: '2-digit', month: '2-digit' })}
                          </div>
                        </div>

                        {/* Macros en ligne sur mobile */}
                        {hasData ? (
                          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', flex: 1, gap: 4 }}>
                            {WEEK_MACROS.map(m => {
                              const val = log?.[m.key] || 0
                              const pct = plan?.[m.target] ? Math.min(100, (val / plan[m.target]) * 100) : 0
                              return (
                                <div key={m.key} style={{ textAlign: 'center' }}>
                                  <div style={{ fontSize: 11, fontWeight: 700, color: m.color }}>{Math.round(val)}</div>
                                  <div style={{ fontSize: 9, color: '#BBB' }}>{m.label}</div>
                                  {plan?.[m.target] && (
                                    <div style={{ height: 2, background: '#F0F0F0', borderRadius: 1, marginTop: 2, overflow: 'hidden' }}>
                          ...
                          <div style={{ flex: 1, fontSize: 12, color: '#CCC', fontStyle: 'italic' }}>
                            {isFuture ? '—' : 'Rien de saisi'}
                          </div>
                        )}

                        {!isFuture && <div style={{ fontSize: 12, color: '#C5D0F0' }}>{isOpen ? '▲' : '▼'}</div>}
                      </div>

                      {/* Drill-down */}
                      {isOpen && !isFuture && (
                        <div style={{ padding: '10px 14px', background: '#F5F8FF' }}>
                          {onOpenDay && (
                            <button onClick={() => onOpenDay(date)} style={{ background: '#0D1B4E', color: 'white', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12, fontWeight: 600, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {sortedWeeks.map(weekStart => {
        ...
        return (
          <div key={weekStart} style={{ background: 'white', borderRadius: 12, border: `1px solid ${isCurrent ? '#C0CAEF' : '#EAEAEA'}`, overflow: 'hidden', boxShadow: '0 2px 6px rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr 1fr 1fr', background: '#F8FAFF', borderBottom: '1px solid #F0F0F0' }}>
              {['Jour', 'Calories', 'Protéines', 'Glucides', 'Lipides'].map(h => (
                <div key={h} style={{ fontSize: 9, letterSpacing: '1px', textTransform: 'uppercase', color: '#999', fontWeight: 600, padding: '6px 10px' }}>{h}</div>
              ))}
            </div>
            ...
            {days.map(({ date, log, isToday, isFuture }) => {
              ...
              return (
                <div key={date}>
                  <div onClick={() => toggle(date, isFuture)} style={{ display: 'grid', gridTemplateColumns: '120px 1fr 1fr 1fr 1fr', borderBottom: '1px solid #F5F5F5', background: isToday ? '#FAFBFF' : isOpen ? '#F5F7FF' : 'transparent', cursor: isFuture ? 'default' : 'pointer' }}
                    ...
                    <div style={{ padding: '9px 10px', fontSize: 12, fontWeight: isToday ? 700 : 500, color: isFuture ? '#CCC' : '#0D1B4E' }}>
                      {isToday ? '📍 ' : ''}{dayName}
                    </div>
                    {WEEK_MACROS.map(m => {
                      const val = log?.[m.key] || 0
                      ...
                      return (
                        <div key={m.key} style={{ padding: '9px 10px' }}>
                          {hasData && val > 0 ? (
                            <>
                              <div style={{ fontSize: 12, fontWeight: 600, color: m.color }}>{Math.round(val)}<span style={{ fontSize: 9, color: '#BBB' }}> {m.unit}</span></div>
                              {plan?.[m.target] && <div style={{ marginTop: 2, height: 3, width: 50, background: '#F0F0F0', borderRadius: 2, overflow: 'hidden' }}><div style={{ height: '100%', background: m.color, width: `${pct}%` }} /></div>}
                            </>
                  ...
                  {isOpen && !isFuture && onOpenDay && (
                    <div style={{ padding: '10px 14px', background: '#F5F8FF', borderBottom: '2px solid #E8ECFA' }}>
                      <button onClick={() => onOpenDay(date)} style={{ background: '#0D1B4E', color: 'white', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12, cursor: 'pointer' }}>📝 Voir/modifier cette journée</button>
                    </div>
                  )}
                </div>
              )
      ...
      {items.length ? (
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
            {items.map((item) => (
              <div key={item.id} style={{ display: 'flex', justifyContent: 'space-between', gap: 10, alignItems: 'center', border: '1px solid #DCE5F3', borderRadius: 10, background: '#FFFFFF', padding: '8px 10px', flexDirection: isMobile ? 'column' : 'row' }}>
                <div style={{ width: '100%' }}>
...
function KpiCard({ label, value, caption }) {
  return (
    <div style={{ border: '1px solid #DCE5F3', borderRadius: 12, background: '#FFFFFF', padding: '10px 12px' }}>
...
function MiniKpi({ label, value }) {
  return <div style={{ border: '1px solid #DCE5F3', borderRadius: 10, background: '#FFFFFF', padding: '8px 10px' }}><div style={{ fontSize: 10, color: '#6B7A99', textTransform: 'uppercase', letterSpacing: '1px', marginBottom: 3 }}>{label}</div><div style={{ fontWeight: 900, fontSize: 18, color: '#0D1B4E' }}>{value}</div></div>
}
