...
async function handler(req, res) {
  ...
  if (await checkRateLimit(req, res, { maxRequests: 10, windowMs: 60_000, routeKey: 'generate-programme' })) return
  ...
  try {
    ...
    if (!response.ok) return res.status(response.status).json({ error: responseData.error?.message || 'Erreur Groq' })
