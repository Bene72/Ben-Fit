 ...
 */
import { isAllowed, isAllowedByKey } from '../../lib/rateLimit'
...
export default async function handler(req, res) {
  ...
  if (email) {
    // Clé par email plutôt que par IP (CORRECTIF AUDIT 09/09/2026, point S3 :
    // isAllowedByKey() prend directement une clé de comptage, on n'a plus
    // besoin de fabriquer une fausse requête pour détourner isAllowed()).
    const emailAllowed = await isAllowedByKey(`login-email:${email}`, {
      maxRequests: 8,
      windowMs: 15 * 60_000,
    })
    if (!emailAllowed) {
