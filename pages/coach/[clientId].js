...
export default function CoachClientNutrition() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      if (profile?.role !== 'coach') { router.push('/'); return }
      setUser(user)
      const { data: cl } = await supabase.from('profiles').select('*').eq('id', clientId).single()
      setClient(cl)
  ...
  return (
    <Layout title="Nutrition client" user={user}>
      {/* Back button */}
      <div style={{ marginBottom:'20px', display:'flex', alignItems:'center', gap:'12px' }}>
        <button onClick={() => router.push('/coach')} style={{ padding:'7px 14px', background:'white', border:'1px solid #E0E0E0', borderRadius:'8px', fontSize:'13px', cursor:'pointer', color:'#666', fontFamily:"'DM Sans',sans-serif" }}>
          ← Retour coach
        </button>
        {client && <span style={{ fontWeight:'700', fontSize:'16px', color:'#0D1B4E' }}>🥗 Nutrition — {client.full_name}</span>}
      </div>
...
function Feedback({ log, plan }) {
  ...
  return (
    <div style={{ marginTop:'12px', padding:'16px 20px', borderRadius:'12px', background:'#FFF8E6', border:'1px solid #F0E0A0' }}>
      <div style={{ fontWeight:'700', fontSize:'13px', color:'#7A5A00', marginBottom:'8px' }}>Feedback</div>
    ...
    <div style={{ textAlign:'center' }}>
      <div style={{ position:'relative', display:'inline-block' }}>
        ...
        <div style={{ position:'absolute', top:'50%', left:'50%', transform:'translate(-50%,-50%)', textAlign:'center', lineHeight:1.2 }}>
          <div style={{ fontWeight:'800', fontSize:'16px', color: over ? '#C45C3A' : '#1A1A14' }}>{value}</div>
...
function FoodDetailBlock({ log, clientId }) {
  ...
  return (
    <div style={{ background:'white', borderRadius:'14px', border:'1px solid #EAEAEA', overflow:'hidden', boxShadow:'0 2px 8px rgba(0,0,0,0.06)' }}>
      <div style={{ padding:'14px 20px', borderBottom:'1px solid #F0F0F0', display:'flex', justifyContent:'space-between', alignItems:'center' }}>
        <div style={{ fontWeight:'700', fontSize:'15px', color:'#0D1B4E' }}>🍽️ Détail des aliments</div>
        {log && <button onClick={() => setShowSearch(!showSearch)} style={{ padding:'5px 12px', background:'#0D1B4E', color:'white', border:'none', borderRadius:'7px', fontSize:'12px', fontWeight:'600', cursor:'pointer' }}>+ Ajouter</button>}
      </div>
      {showSearch && log && (
        <div style={{ padding:'16px 20px', background:'#F8F9FC', borderBottom:'1px solid #EAEAEA' }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 100px 180px', gap:'10px', marginBottom:'10px' }}>
            <div style={{ position:'relative' }}>
              ...
              <input value={query} onChange={e => { setQuery(e.target.value); setSelected(null) }} placeholder="Ex: poulet, riz…" style={inp} />
      ...
      {items.length === 0 ? (
        <div style={{ padding:'24px', textAlign:'center', color:'#CCC', fontSize:'13px' }}>
          {log ? 'Aucun aliment ajouté' : "Saisis d'abord les apports du jour"}
        </div>
      ) : (
