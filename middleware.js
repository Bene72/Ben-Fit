 ...
 *
 * ⚠️ REVERT D'URGENCE (09/09/2026) : le CSP par nonce (point S4 de l'audit
 * du 09/09) a été retiré. En production sur Vercel, le middleware semble
 * s'exécuter plus d'une fois par requête dans certains cas, générant deux
 * nonces différents dans un seul en-tête Content-Security-Policy — Chrome
 * bloquait alors le script de démarrage de Next.js lui-même (site figé sur
 * "CHARGEMENT…" indéfiniment). Cause exacte non confirmée (comportement
 * d'infrastructure Vercel non reproductible en local). Un CSP statique
 * fonctionnel vaut mieux qu'un CSP plus strict qui casse le site — à
 * ré-essayer plus tard via un déploiement Preview, testé en conditions
