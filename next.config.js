...
const securityHeaders = [
  ...
  // (hydration data).
  //
  // ⚠️ REVERT D'URGENCE (09/09/2026) : une version avec nonce par requête
  // (générée dans middleware.js) a été tentée pour retirer 'unsafe-inline'
  // sur script-src, mais a cassé le site en production sur Vercel (double
  // nonce dans l'en-tête, script de démarrage de Next.js bloqué). Retour à
  // ce CSP statique, connu pour fonctionner. Voir middleware.js pour le
  // détail de l'incident.
  {
    ...
    value: [
      ...
      `connect-src 'self' https://${SUPABASE_DOMAIN} wss://${SUPABASE_DOMAIN} https://api.anthropic.com`,
